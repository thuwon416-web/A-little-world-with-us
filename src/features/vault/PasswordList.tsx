...
export default function PasswordList({ masterKey }: PasswordListProps) {
  ...
  return (
    <section>
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-2xl text-text-1" style={{ fontFamily: 'var(--font-display)' }}>Passwords</h2><p className="mt-1 text-sm text-text-2">Encrypted credentials for the two of you.</p></div><div className="flex flex-wrap gap-2"><button type="button" onClick={() => setHealthOpen((value) => !value)} className="inline-flex items-center gap-1 rounded-btn border border-accent-1/20 px-3 py-2 text-xs text-text-2"><HeartPulse className="h-4 w-4" />Health</button><button type="button" onClick={() => void download()} className="rounded-btn border border-accent-1/20 p-2 text-text-2" aria-label="Export passwords"><Download className="h-4 w-4" /></button><label className="cursor-pointer rounded-btn border border-accent-1/20 p-2 text-text-2" aria-label="Import passwords"><Upload className="h-4 w-4" /><input type="file" accept=".vault,application/json" className="hidden" onChange={(event) => void upload(event)} /></label><button type="button" onClick={() => void load()} className="rounded-btn border border-accent-1/20 p-2 text-text-2" aria-label="Refresh passwords"><RefreshCw className="h-4 w-4" /></button><button type="button" onClick={() => setEditing({})} className="inline-flex items-center gap-2 rounded-btn bg-accent-1 px-4 py-2 text-sm text-white"><Plus className="h-4 w-4" />Add password</button></div></div>
      {healthOpen ? <PasswordHealth credentials={credentials} masterKey={masterKey} onFocus={setHighlighted} /> : null}
      {error ? <p className="mb-4 rounded-btn bg-red-400/10 p-4 text-sm text-red-200">{error}</p> : null}
      {credentials.length === 0 ? <div className="rounded-modal border border-dashed border-accent-1/30 p-10 text-center text-sm text-text-2">No passwords yet</div> : <div className="grid gap-4 md:grid-cols-2">{credentials.map((credential) => <PasswordCard key={credential.id} credential={credential} masterKey={masterKey} highlighted={highlighted.includes(credential.id)} onEdit={(item, data) => setEditing({ credential: item, data })} onDelete={(id) => void remove(id)} />)}</div>}
