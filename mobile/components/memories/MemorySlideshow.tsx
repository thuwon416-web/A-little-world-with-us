      ...
      <View style={[styles.container, { backgroundColor: colors.background }]}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <View style={styles.slide}>
              {urls[item.id] ? (
                <Image source={{ uri: urls[item.id] }} style={styles.image} resizeMode="contain" />
              ) : (
                <Text style={{ color: colors.textSecondary }}>Loading photo…</Text>
              )}
              <View style={[styles.caption, { backgroundColor: colors.cardBg }]}>
                <Text style={[styles.title, { color: colors.textPrimary }]}>
                  {item.title || 'A memory together'}
                </Text>
        ...
        <View style={styles.controls}>
          <TouchableOpacity
            ...
            style={[styles.control, { backgroundColor: colors.cardBg }]}
            accessibilityLabel={playing ? 'Pause slideshow' : 'Play slideshow'}
          >
            {playing ? (
              <Pause color={colors.textPrimary} size={18} />
            ) : (
              <Play color={colors.textPrimary} size={18} />
            )}
          </TouchableOpacity>
          <View style={styles.dots}>
            {photos.map((photo, photoIndex) => (
              <View
                ...
                style={[
                  styles.dot,
                  { backgroundColor: photoIndex === index ? colors.accent1 : colors.cardBorder },
                ]}
              />
            ))}
          </View>
