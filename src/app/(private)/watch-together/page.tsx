  ...
  return <main className="mx-auto max-w-7xl space-y-6 p-4 md:p-8">
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      <section className="space-y-4">
        ...
        <form onSubmit={(event) => void add(event)} className="glass-card grid gap-3 rounded-2xl p-4 md:grid-cols-[1fr_1fr_auto]"><input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="YouTube URL or video ID" className="rounded-lg border border-white/10 bg-black/20 px-3 py-2" /><input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Title (optional)" className="rounded-lg border border-white/10 bg-black/20 px-3 py-2" /><button className="rounded-lg border border-[#ff6b81] bg-transparent px-4 py-2 text-[#ff6b81] hover:bg-[#ff6b81]/10" type="submit">Add video</button></form>
      </section>
