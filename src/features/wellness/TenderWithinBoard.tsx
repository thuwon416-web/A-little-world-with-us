...
const starterItems: WithinItem[] = [
  {
    id: 'within-1',
    title: 'Choose an inward softness before an outward answer',
    tone: 'ground',
    done: true,
  },
  {
    id: 'within-2',
    title: 'Name one honest feeling without turning it into a verdict',
    tone: 'within',
    done: false,
  },
  {
    id: 'within-3',
    title: 'Let tenderness be a steadier guide than urgency',
    tone: 'ease',
    done: true,
  },
]
...
export default function TenderWithinBoard() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('tender-within-board')
      if (raw) {
        const parsed = JSON.parse(raw)
        if (Array.isArray(parsed) && parsed.length) setItems(parsed)
      }
      const savedPrompt = localStorage.getItem('tender-within-prompt')
      if (savedPrompt) {
  ...
  useEffect(() => {
    localStorage.setItem('tender-within-board', JSON.stringify(items))
  }, [items])

  useEffect(() => {
    localStorage.setItem('tender-within-prompt', String(promptIndex))
  }, [promptIndex])
  ...
  const addItem = () => {
    ...
    setItems((current) => [
      ...current,
      { id: `within-${Date.now()}`, title: value, tone, done: false },
    ])
    setTitle('')
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-[var(--accent-2)]">
          ...
          <h3 className="font-dancing text-2xl">Tender Within</h3>
        </div>
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-gradient-to-r from-[var(--accent-2)] to-[var(--accent-1)] p-3 text-sm text-[var(--text-primary)]/80">
        ...
        <p>
          When the world feels loud, tenderness inside is the quiet place where the next kind choice
          can begin.
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 rounded-2xl border border-[var(--accent-1)]/30 bg-[var(--bg-2)] p-3 text-sm text-[var(--accent-1)]">
        ...
        <span>Let tenderness be a steadier guide than urgency.</span>
      </div>
    </div>
  )
}
