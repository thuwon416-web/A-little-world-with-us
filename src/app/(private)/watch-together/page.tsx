...
export default function WatchTogetherPage() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-4 md:p-8">
    <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-sm uppercase tracking-[0.25em] text-text-2">Shared moment</p><h1 className="text-4xl font-bold">Watch Together</h1><p className="mt-2 text-text-2">Choose a video and stay in sync with your partner.</p></div><Link href="/chat" className="glass-button rounded-xl px-4 py-2">Open full chat</Link></div>
    {error && <p className="rounded-xl bg-error/10 p-3 text-error">{error}</p>}
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      <section className="space-y-4">
        <div className="glass-card overflow-hidden rounded-btn p-3">{current ? <><div className="aspect-video overflow-hidden rounded-xl bg-black"><iframe className="h-full w-full" src={`https://www.youtube.com/embed/${current.youtube_id}?enablejsapi=1&cc_load_policy=1`} title={current.title} allow="autoplay; encrypted-media; picture-in-picture" allowFullScreen /></div><div className="flex items-center justify-between gap-3 p-2"><h2 className="font-semibold">{current.title}</h2><button className="glass-button rounded-lg px-4 py-2" onClick={() => void toggle()}>{playing ? 'Pause sync' : 'Play sync'}</button></div></> : <div className="p-12 text-center text-text-2">Add a video to start your watch party.</div>}</div>
