...
const starterItems: HoldItem[] = [
  { id: 'hold-1', title: 'Leave a little room for the feelings to settle', tone: 'hold', done: true },
  { id: 'hold-2', title: 'Offer one real comfort instead of a quick fix', tone: 'warmth', done: false },
  { id: 'hold-3', title: 'Keep the space gentle enough to breathe', tone: 'calm', done: true },
]

const toneMeta: Record<HoldItem['tone'], { label: string; backgroundColor: string; borderColor: string; color: string }> = {
...
export default function GentleHoldBoard() {
  ...
  const addItem = () => {
    ...
    setItems((current) => [...current, { id: `hold-${Date.now()}`, title: value, tone, done: false }])
    setTitle('')
  }
  ...
  return (
    <WellnessBoardShell title="Gentle Hold" subtitle="Support rhythm" badge="hold">
      ...
      <View style={styles.form}>
        ...
        <TextInput value={title} onChangeText={setTitle} placeholder="Add a holding cue" placeholderTextColor="#8f8393" style={styles.input} />
