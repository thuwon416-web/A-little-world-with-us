...
export const relationshipMemoriesService = {
  ...
  async getByCategory(coupleId: string, category: string): Promise<RelationshipMemory[]> {
    const { data, error } = await supabase
      .from('relationship_memories')
      .select('*')
      .eq('couple_id', coupleId)
  ...
  async search(coupleId: string, keyword: string): Promise<RelationshipMemory[]> {
    const { data, error } = await supabase
      .from('relationship_memories')
      .select('*')
      .eq('couple_id', coupleId)
      .or(`quote_burmese.ilike.%${keyword}%,context.ilike.%${keyword}%`)
      .order('date_time', { ascending: false })
    if (error) throw error
    return (data ?? []) as RelationshipMemory[]
  },
  ...
  async getOnThisDay(coupleId: string, date: Date): Promise<RelationshipMemory[]> {
    const { data, error } = await supabase
      ...
      .order('date_time', { ascending: false })
    if (error) throw error
    return ((data ?? []) as RelationshipMemory[]).filter((memory) => {
