...
type EmberItem = { id: string; feeling: string; tone: 'ember' | 'slow' | 'safe' | 'still'; done: boolean }

const starterItems: EmberItem[] = [
  { id: 'ember-1', feeling: 'Let a small tender feeling sit without rushing it away', tone: 'ember', done: true },
  { id: 'ember-2', feeling: 'Give yourself the space to feel quietly what is real', tone: 'slow', done: false },
  { id: 'ember-3', feeling: 'Hold what is fragile with the gentleness it deserves', tone: 'safe', done: true },
]

const toneMeta: Record<EmberItem['tone'], { label: string; style: { backgroundColor: string; borderColor: string } }> = {
  ember: { label: 'ember', style: { backgroundColor: '#271d22', borderColor: '#f4c7a5' } },
  slow: { label: 'slow', style: { backgroundColor: '#1c2129', borderColor: '#b0d8c5' } },
  safe: { label: 'safe', style: { backgroundColor: '#2a2131', borderColor: '#d8b9c8' } },
  still: { label: 'still', style: { backgroundColor: '#1a1d24', borderColor: '#d9e2ff' } },
}

const prompts = [
  'What quiet feeling has been asking for more space to breathe?',
  'Where can you let yourself be tender without judgment?',
  'What small hurt is ready to be held more gently?',
  'How can this moment be a quiet sanctuary for what you need?',
]

export default function QuietEmberBoard() {
  const [items, setItems] = useState<EmberItem[]>(starterItems)
  const [feeling, setFeeling] = useState('')
  const [tone, setTone] = useState<EmberItem['tone']>('ember')
  ...
  const addItem = () => {
    ...
    setItems((current) => [...current, { id: `ember-${Date.now()}`, feeling: value, tone, done: false }])
    setFeeling('')
  }
  ...
  return (
    <WellnessBoardShell title="Quiet Ember" subtitle="Ember care" badge="ember">
      <View style={styles.progressCard}>
        <View style={styles.progressHeader}>
          <Text style={styles.progressLabel}>Ember care</Text>
        ...
        {items.map((item) => (
          <Pressable key={item.id} onPress={() => toggleItem(item.id)} style={[styles.item, item.done && styles.itemDone]}>
            <View style={styles.itemInner}>
              ...
              <Text style={[styles.itemText, item.done && styles.itemTextDone]}>{item.feeling}</Text>
            </View>
      ...
      <View style={styles.form}>
        <View style={styles.optionRow}>
          {(['ember', 'slow', 'safe', 'still'] as EmberItem['tone'][]).map((option) => (
            <Pressable key={option} onPress={() => setTone(option)} style={[styles.option, option === tone && styles.optionSelected]}>
              <Text style={[styles.optionText, option === tone && styles.optionTextSelected]}>{option}</Text>
            </Pressable>
          ))}
        </View>
        <TextInput value={feeling} onChangeText={setFeeling} placeholder="What quiet feeling needs care?" placeholderTextColor="#8f8393" style={styles.input} />
