...
type Item = { id: string; title: string; tone: "open" | "warmth" | "ease" | "calm"; done: boolean }
...
const toneMeta: Record<Item["tone"], { label: string; style: { backgroundColor: string; borderColor: string } }> = {
  open: { label: "open", style: { backgroundColor: "#2d2234", borderColor: "#d8b9c8" } },
  warmth: { label: "warmth", style: { backgroundColor: "#2d2234", borderColor: "#d8b9c8" } },
  ease: { label: "ease", style: { backgroundColor: "#1c2a25", borderColor: "#b0d8c5" } },
  calm: { label: "calm", style: { backgroundColor: "#2d2234", borderColor: "#d8b9c8" } },
}

const prompts = [
  "What open gesture lets the other person feel supported without pressure?",
  "Which small act of care feels more natural than a big gesture?",
  "What gentle phrase helps the moment stay soft and honest?",
  "How can we offer help without making the room feel heavier?",
]

export default function OpenHandBoard() {
  const [items, setItems] = useState<Item[]>(starterItems)
  const [title, setTitle] = useState("")
  const [tone, setTone] = useState<Item["tone"]>("open")
  ...
  return (
    <WellnessBoardShell title="Open Hand" subtitle="Care rhythm" badge="connection">
      <View style={styles.progressCard}>
        <View style={styles.progressHeader}>
          <Text style={styles.progressLabel}>Care rhythm</Text>
      ...
      <View style={styles.form}>
        <View style={styles.optionRow}>
          {(["open", "warmth", "ease", "calm"] as Item["tone"][]).map((option) => (
        ...
        <TextInput value={title} onChangeText={setTitle} placeholder="Add a gesture" placeholderTextColor="#8f8393" style={styles.input} />
