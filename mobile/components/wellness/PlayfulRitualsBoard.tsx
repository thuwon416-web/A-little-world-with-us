...
type PlayItem = { id: string; title: string; mood: 'light' | 'joy' | 'ease' | 'wonder'; done: boolean }

const starterItems: PlayItem[] = [
  { id: 'playful-1', title: 'Find one moment that makes you both laugh', mood: 'light', done: true },
  { id: 'playful-2', title: 'Let spontaneous joy arrive without planning', mood: 'joy', done: false },
  { id: 'playful-3', title: 'Play with ease instead of pressure', mood: 'ease', done: true },
]

const moodMeta: Record<PlayItem['mood'], { label: string; style: { backgroundColor: string; borderColor: string } }> = {
...
export default function PlayfulRitualsBoard() {
  ...
  const addItem = () => {
    ...
    if (!value) return
    setItems((current) => [...current, { id: `playful-${Date.now()}`, title: value, mood, done: false }])
    setTitle('')
  }
  ...
  return (
    <WellnessBoardShell title="Playful Rituals" subtitle="Play log" badge="playful">
      ...
      <View style={styles.form}>
        ...
        <TextInput value={title} onChangeText={setTitle} placeholder="Add a playful moment" placeholderTextColor="#8f8393" style={styles.input} />
