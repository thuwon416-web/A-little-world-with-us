...
type ForecastItem = { id: string; title: string; kind: 'sun' | 'rain' | 'cozy' | 'clear'; done: boolean }

const starterItems: ForecastItem[] = [
  { id: 'forecast-1', title: 'A morning where nothing feels rushed', kind: 'sun', done: true },
  { id: 'forecast-2', title: 'A calm evening with enough room to breathe', kind: 'rain', done: false },
  { id: 'forecast-3', title: 'A clear signal that we are still choosing each other', kind: 'clear', done: true },
]

const kindMeta: Record<ForecastItem['kind'], { label: string; style: { backgroundColor: string; borderColor: string } }> = {
  sun: { label: 'sun', style: { backgroundColor: '#2d2234', borderColor: '#d8b9c8' } },
  rain: { label: 'rain', style: { backgroundColor: '#171d29', borderColor: '#b7c3f0' } },
  cozy: { label: 'cozy', style: { backgroundColor: '#271d22', borderColor: '#f4c7a5' } },
  clear: { label: 'clear', style: { backgroundColor: '#1c2129', borderColor: '#b0d8c5' } },
}
...
export default function GentleForecastBoard() {
  ...
  const addItem = () => {
    ...
    setItems((current) => [...current, { id: `forecast-${Date.now()}`, title: value, kind, done: false }])
    setTitle('')
  }
  ...
  return (
    <WellnessBoardShell title="Together Forecast" subtitle="Forecast mood" badge="forecast">
      <View style={styles.progressCard}>
        <View style={styles.progressHeader}>
          <Text style={styles.progressLabel}>Forecast mood</Text>
      ...
      <View style={styles.form}>
        ...
        <TextInput value={title} onChangeText={setTitle} placeholder="Add a tomorrow feeling" placeholderTextColor="#8f8393" style={styles.input} />
      ...
      <View style={styles.promptCard}>
        <View style={styles.promptHeader}>
          <Text style={styles.promptLabel}>Forecast prompt</Text>
