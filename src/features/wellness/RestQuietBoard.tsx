...
type RestItem = {
  id: string
  title: string
  tone: 'rest' | 'warmth' | 'ease' | 'calm'
  done: boolean
}

const starterItems: RestItem[] = [
  {
    id: 'rest-1',
    title: 'Let the final hour soften instead of speeding up',
    tone: 'rest',
    done: true,
  },
  {
    id: 'rest-2',
    title: 'Choose one drowsy comfort cue for the night',
    tone: 'warmth',
    done: false,
  },
  { id: 'rest-3', title: 'Keep the evening loose enough to settle', tone: 'calm', done: true },
]
...
const prompts = [
  'What quiet rhythm helps the body settle into rest?',
  'Which tenderness makes the night feel safer and softer?',
  'How can the evening be gentle enough not to overrun the heart?',
  'What resting cue brings the nervous system back to ease?',
]

export default function RestQuietBoard() {
  const [items, setItems] = useState<RestItem[]>(starterItems)
  const [title, setTitle] = useState('')
  const [tone, setTone] = useState<RestItem['tone']>('rest')
  const [promptIndex, setPromptIndex] = useState(0)

  useEffect(() => {
    try {
      const raw = localStorage.getItem('rest-quiet-board')
      if (raw) {
        const parsed = JSON.parse(raw)
        if (Array.isArray(parsed) && parsed.length) setItems(parsed)
      }
      const savedPrompt = localStorage.getItem('rest-quiet-prompt')
      if (savedPrompt) {
  ...
  useEffect(() => {
    localStorage.setItem('rest-quiet-board', JSON.stringify(items))
  }, [items])

  useEffect(() => {
    localStorage.setItem('rest-quiet-prompt', String(promptIndex))
  }, [promptIndex])
  ...
  const addItem = () => {
    ...
    setItems((current) => [
      ...current,
      { id: `rest-${Date.now()}`, title: value, tone, done: false },
    ])
    setTitle('')
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-[var(--accent-2)]">
          ...
          <h3 className="font-dancing text-2xl">Rest Quiet</h3>
        </div>
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-[var(--card-bg)]/15 p-3">
        <div className="mb-2 flex gap-2 overflow-x-auto">
          {(['rest', 'warmth', 'ease', 'calm'] as RestItem['tone'][]).map((option) => (
            <button
              key={option}
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="Add a night rest cue"
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-gradient-to-r from-[var(--accent-2)] to-[var(--accent-1)] p-3 text-sm text-[var(--text-primary)]/80">
        ...
        <p>
          Rest is wiser when it is given space to arrive gently, without becoming a demand or a
          performance.
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 rounded-2xl border border-[var(--accent-1)]/30 bg-[var(--bg-2)] p-3 text-sm text-[var(--accent-1)]">
        ...
        <span>Let the body and heart land softly tonight.</span>
      </div>
    </div>
  )
}
