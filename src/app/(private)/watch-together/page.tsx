...
export default function WatchTogetherPage() {
  ...
  const broadcast = async (event: 'video', payload: Record<string, unknown> = {}) => {
  ...
  const select = async (item: WatchItem) => {
    setCurrent(item); await broadcast('video', { youtubeId: item.youtube_id })
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-4 md:p-8">
    <div className="flex flex-wrap items-end justify-between gap-3"><div><p className="text-sm uppercase tracking-[0.25em] text-text-2">Shared moment</p><h1 className="text-4xl font-bold">Watch Together</h1><p className="mt-2 text-text-2">ရွေးထားတဲ့ဗီဒီယိုက နှစ်ယောက်စလုံးရဲ့စာရင်းမှာ တစ်ပြိုင်တည်းပြောင်းပါမယ်။ ဝဘ်မှာ ဖွင့်ခြင်း၊ ရပ်ခြင်းနဲ့ အချိန်တူအောင်ချိန်ခြင်း မရသေးပါ။</p></div><Link href="/chat" className="glass-button rounded-xl px-4 py-2">Open full chat</Link></div>
    {error && <p className="rounded-xl bg-error/10 p-3 text-error">{error}</p>}
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      <section className="space-y-4">
        <div className="glass-card overflow-hidden rounded-btn p-3">{current ? <><div className="aspect-video overflow-hidden rounded-xl bg-black"><iframe className="h-full w-full" src={`https://www.youtube.com/embed/${current.youtube_id}?cc_load_policy=1`} title={current.title} allow="autoplay; encrypted-media; picture-in-picture" allowFullScreen /></div><div className="p-2"><h2 className="font-semibold">{current.title}</h2></div></> : <div className="p-12 text-center text-text-2">Add a video to start your watch party.</div>}</div>
        <form onSubmit={(event) => void add(event)} className="glass-card grid gap-3 rounded-btn p-4 md:grid-cols-[1fr_1fr_auto]"><input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="YouTube URL or video ID" className="rounded-lg border border-border bg-card/20 px-3 py-2" /><input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Title (optional)" className="rounded-lg border border-border bg-card/20 px-3 py-2" /><button className="rounded-lg border border-accent-1 bg-transparent px-4 py-2 text-accent-1 hover:bg-accent-1/10" type="submit">Add video</button></form>
      </section>
      <aside className="space-y-4"><div className="glass-card rounded-btn p-4"><h2 className="mb-3 text-xl font-semibold">Watchlist</h2><div className="space-y-3">{items.map((item) => <div key={item.id} className="flex items-center gap-3"><img src={item.thumbnail_url ?? getYouTubeThumbnail(item.youtube_id)} alt="" className="h-12 w-20 rounded object-cover" /><button className="min-w-0 flex-1 text-left" onClick={() => void select(item)}><span className="block truncate font-medium">{item.title}</span><span className="text-xs text-text-2">Play together</span></button><button className="text-xs text-error" onClick={() => void supabase.from('watchlist').delete().eq('id', item.id).then(() => setItems((old) => old.filter((row) => row.id !== item.id)))}>Remove</button></div>)}</div></div><div className="glass-card max-h-[500px] overflow-auto rounded-btn p-3"><h2 className="mb-2 px-2 text-xl font-semibold">Party chat</h2><RealtimeChat /></div></aside>
    </div>
  </div>
}
