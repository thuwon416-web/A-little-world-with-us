  ...
  return (
    <div className="space-y-4">
      ...
      <button
        ...
        onClick={handleSOS}
        disabled={isSending}
        className="relative w-full overflow-hidden rounded-btn border-2 border-red-500 bg-gradient-to-br from-red-600 to-red-700 p-6 text-white shadow-lg transition-all hover:shadow-xl hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:hover:scale-100"
      >
        <div className="flex items-center justify-center gap-3">
          {isSending ? (
            <>
              <div className="h-6 w-6 animate-spin rounded-full border-2 border-border/50 border-t-white" />
      ...
      {lastSOS && (
        <div className="rounded-xl border border-red-500/20 bg-red-500/5 p-4">
          <div className="flex items-center gap-2 mb-2">
            ...
            <p className="text-xs text-text-2">Last SOS sent</p>
          </div>
      ...
      {location && (
        <div className="rounded-xl border border-accent-1/20 bg-card p-4">
          <div className="flex items-center gap-2 mb-2">
            <MapPin className="h-4 w-4 text-accent-1" />
            <p className="text-xs text-text-2">Location shared</p>
          </div>
          <p className="text-sm text-text-1">
      ...
      <div className="rounded-xl border border-accent-1/20 bg-card p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-text-1">Emergency Information</p>
      ...
      {('Notification' in window && Notification.permission === 'default') && (
        <button
          ...
          className="w-full rounded-lg border border-accent-1/20 px-4 py-2 text-sm text-accent-1 transition hover:bg-accent-1/10"
        >
