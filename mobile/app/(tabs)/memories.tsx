...
export default function MemoriesScreen() {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <TouchableOpacity style={styles.secondary} onPress={() => router.push('/(tabs)/gallery')}>
        <Text style={styles.secondaryText}>Open photo gallery</Text>
      </TouchableOpacity>
      <ScrollView
        horizontal
      ...
      >
        {categories.map((category) => (
          <TouchableOpacity
            ...
            style={[styles.filter, filter === category && styles.filterActive]}
          >
      ...
      {visible.length === 0 ? (
        <Text style={styles.muted}>No structured memories in this category yet.</Text>
      ) : (
        visible.map((memory) => (
...
const styles = StyleSheet.create({
  ...
  secondary: { borderRadius: 12, padding: 12, backgroundColor: '#1f3b2f', alignItems: 'center' },
  ...
  filterActive: { backgroundColor: '#d8b9c8' },
  filterText: { color: '#f3f0f5', textTransform: 'capitalize' },
  card: {
    backgroundColor: '#171b22',
    borderRadius: 18,
    borderWidth: 1,
  ...
  memoryTitle: { color: '#f3f0f5', fontSize: 18, fontWeight: '700', flex: 1 },
  meta: { color: '#d9bfd7', textTransform: 'capitalize', fontSize: 12 },
  caption: { color: '#c4c4ce', lineHeight: 21 },
  muted: { color: '#c4c4ce' },
  error: { color: '#ff9b9b' },
  delete: { color: '#ff9b9b', fontWeight: '700' },
})
