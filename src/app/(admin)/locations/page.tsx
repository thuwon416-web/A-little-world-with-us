  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_330px]">
      <section className="glass-card min-h-[520px] overflow-hidden p-4">
        ...
        {tab === 'places' && <div className="space-y-3 p-2"><h2 className="text-xl text-[var(--text-primary)]">Saved places</h2><div className="grid gap-2 rounded-2xl bg-[var(--bg-2)] p-4 sm:grid-cols-2"><input value={placeForm.name} onChange={(event) => setPlaceForm((form) => ({ ...form, name: event.target.value }))} placeholder="Name" className="rounded-xl border border-[var(--card-border)] bg-[var(--card-bg)] px-3 py-2 text-sm text-[var(--text-primary)]" /><input value={placeForm.latitude} disabled={Boolean(editingPlaceId)} onChange={(event) => setPlaceForm((form) => ({ ...form, latitude: event.target.value }))} placeholder="Latitude" className="rounded-xl border border-[var(--card-border)] bg-[var(--card-bg)] px-3 py-2 text-sm text-[var(--text-primary)]" /><input value={placeForm.longitude} disabled={Boolean(editingPlaceId)} onChange={(event) => setPlaceForm((form) => ({ ...form, longitude: event.target.value }))} placeholder="Longitude" className="rounded-xl border border-[var(--card-border)] bg-[var(--card-bg)] px-3 py-2 text-sm text-[var(--text-primary)]" /><input value={placeForm.radius} onChange={(event) => setPlaceForm((form) => ({ ...form, radius: event.target.value }))} placeholder="Radius (25-10000m)" className="rounded-xl border border-[var(--card-border)] bg-[var(--card-bg)] px-3 py-2 text-sm text-[var(--text-primary)]" /><button type="button" onClick={() => void savePlace()} className="glass-button sm:col-span-2">{editingPlaceId ? 'Update place' : 'Add place'}</button>{editingPlaceId ? <button type="button" onClick={cancelEditPlace} className="text-sm text-[var(--text-secondary)] sm:col-span-2">Cancel edit</button> : null}</div>{places.map((place) => <div key={place.id} className="flex items-center justify-between rounded-2xl bg-[var(--bg-2)] p-4"><div><p className="font-medium text-[var(--text-primary)]">{place.name}</p><p className="mt-1 text-sm text-[var(--text-secondary)]">{place.latitude.toFixed(5)}, {place.longitude.toFixed(5)} · {place.radius_meters}m radius</p></div><div className="flex gap-3"><button type="button" onClick={() => editPlace(place)} className="text-sm text-[var(--accent-1)]">Edit</button><button type="button" onClick={() => void deletePlace(place.id)} className="text-sm text-[var(--error)]">Delete</button></div></div>)}{!places.length ? <EmptyPanel title="No saved places" text="Add a safe zone above." /> : null}</div>}
      ...
      <aside className="space-y-3">{profiles.map((profile) => { const location = locations.find((row) => row.user_id === profile.id); const stale = !location || Date.now() - new Date(location.updated_at).getTime() > 10 * 60000; return <button key={profile.id} type="button" onClick={() => setSelectedUser(profile.id)} className={`w-full rounded-2xl border p-4 text-left ${selectedUser === profile.id ? 'border-[var(--accent-1)]/50 bg-[var(--accent-1)]/10' : 'border-[var(--card-border)] bg-[var(--card-bg)]'}`}><p className="font-medium text-[var(--text-primary)]">{profile.full_name || profile.email}</p><p className={`mt-1 text-xs ${stale ? 'text-[var(--warning)]' : 'text-[var(--success)]'}`}>{stale ? 'Location stale or offline' : `Live · ${relativeTime(location.updated_at)}`}</p><p className="mt-2 text-xs text-[var(--text-secondary)]">{location ? `Battery ${location.battery_level ?? '—'}% · ${location.is_charging ? 'Charging' : 'Not charging'} · ${location.network_type ?? 'Network unknown'}` : 'No GPS update yet'}</p></button>})}</aside>
    </div>
  </div>
}
...
function SafetyPanel({ alerts, profiles, checkins, onChanged }: { alerts: EmergencyAlert[]; profiles: Profile[]; checkins: SafetyCheckin[]; onChanged: () => void }) {
  ...
  return <div className="space-y-4 p-2"><EmergencySOS onSOS={() => onChanged()} /><div className="grid grid-cols-3 gap-3"><button type="button" onClick={() => void startCheckin('safe', 'I am safe')} className="rounded-xl bg-[var(--success)] p-3 text-sm font-medium text-[var(--bg-color)]">I am safe</button><button type="button" onClick={() => void startCheckin('need_help', 'I need help')} className="rounded-xl bg-[var(--error)] p-3 text-sm font-medium text-[var(--text-primary)]">I need help</button><button type="button" onClick={() => void startCheckin('home', 'I am home')} className="rounded-xl bg-[var(--accent-1)] p-3 text-sm font-medium text-[var(--bg-color)]">I am home</button></div>{checkins.slice(0, 5).map((checkin) => <div key={checkin.id} className="rounded-xl bg-[var(--bg-2)] p-3 text-sm text-[var(--text-secondary)]"><p className="font-medium text-[var(--text-primary)]">{checkin.checkinType.replace('_', ' ')} · {checkin.status}</p><p>{new Date(checkin.createdAt).toLocaleString()}{checkin.expectedUntil ? ` · expected by ${new Date(checkin.expectedUntil).toLocaleString()}` : ''}</p></div>)}<h2 className="text-xl text-[var(--text-primary)]">Recent SOS alerts</h2>{alerts.length ? alerts.map((alert) => <div key={alert.id} className="block rounded-2xl border border-[var(--error)]/30 bg-[var(--error)]/10 p-4"><a href={alert.latitude !== null && alert.longitude !== null ? `https://www.google.com/maps?q=${alert.latitude},${alert.longitude}` : '#'} target="_blank" rel="noreferrer"><p className="font-medium text-[var(--error)]">🆘 {profiles.find((profile) => profile.id === alert.reporter_id)?.full_name || 'Partner'} sent an SOS</p><p className="mt-1 text-sm text-[var(--text-secondary)]">{new Date(alert.created_at).toLocaleString()} · {alert.latitude !== null ? 'open map' : 'location unavailable'}</p></a>{alert.resolved_at ? <div className="mt-3 text-sm text-[var(--text-secondary)]"><p>Resolved by {profiles.find((profile) => profile.id === alert.resolved_by)?.full_name || 'Partner'} on {new Date(alert.resolved_at).toLocaleString()}</p>{alert.resolution_note ? <p className="mt-1">Note: {alert.resolution_note}</p> : null}</div> : <button type="button" onClick={() => void handleResolve(alert)} className="mt-3 rounded-lg bg-[var(--success)] px-3 py-2 text-sm font-medium text-[var(--bg-color)]">Resolve</button>}</div>) : <p className="rounded-2xl bg-[var(--bg-2)] p-4 text-sm text-[var(--text-secondary)]">No SOS alerts yet.</p>}</div>
}
