...
export type SafetyCheckin = {
  ...
  coupleId: string
  userId: string
  checkinType: 'safe' | 'need_help' | 'home'
...
export type SafetyCheckinInput = Pick<
  SafetyCheckin,
  'checkinType' | 'message' | 'latitude' | 'longitude' | 'accuracy' | 'expectedUntil'
>
...
const mapCheckin = (r: Row): SafetyCheckin => ({
  ...
  checkinType: r.checkin_type as SafetyCheckin['checkinType'],
  status: r.status as SafetyCheckin['status'],
  message: r.message as string | null,
  latitude: r.latitude as number | null,
  longitude: r.longitude as number | null,
  accuracy: r.accuracy as number | null,
  expectedUntil: r.expected_until as string | null,
...
export async function getCheckins() {
  ...
  return ((data ?? []) as Row[]).map(mapCheckin)
}
export async function createCheckin(input: SafetyCheckinInput) {
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      expected_until: input.expectedUntil,
    })
    .select('*')
    .single()
  if (error) throw new Error(error.message)
...
export async function acknowledgeCheckin(id: string) {
  const { coupleId, user } = await context()
