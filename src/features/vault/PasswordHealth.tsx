...
export default function PasswordHealth({ credentials, masterKey, onFocus }: { credentials: VaultCredential[]; masterKey: Uint8Array; onFocus: (ids: string[]) => void }) {
  ...
  return <div className="mb-5 rounded-panel border border-accent-1/20 bg-card p-5"><div className="flex items-center gap-4"><div className="flex h-16 w-16 items-center justify-center rounded-full border-4 border-accent-1 text-xl text-text-1">{score}</div><div><h3 className="text-lg text-text-1">Password health</h3><p className="text-xs text-text-2">{health.secure} of {total} passwords pass all checks</p></div></div><div className="mt-4 grid gap-2 sm:grid-cols-3">{categories.map(([label, items]) => <button key={label} type="button" onClick={() => onFocus(items.map((item) => item.id))} className="rounded-btn bg-soft-tint p-3 text-left"><p className="text-xs text-text-2">{label}</p><p className="mt-1 text-xl text-text-1">{items.length}</p><span className="text-xs text-accent-1">Fix</span></button>)}</div></div>
}
