...
type LandingItem = { id: string; title: string; kind: 'ground' | 'safe' | 'rest' | 'stable'; done: boolean }

const starterItems: LandingItem[] = [
  { id: 'landing-1', title: 'Find a grounded moment that holds steady', kind: 'ground', done: true },
  { id: 'landing-2', title: 'Let yourself land safely without needing to keep moving', kind: 'safe', done: false },
  { id: 'landing-3', title: 'Build a stable foundation for what comes next', kind: 'stable', done: true },
]

const kindMeta: Record<LandingItem['kind'], { label: string; style: { backgroundColor: string; borderColor: string } }> = {
  ground: { label: 'ground', style: { backgroundColor: '#1c2129', borderColor: '#b0d8c5' } },
  safe: { label: 'safe', style: { backgroundColor: '#2d2234', borderColor: '#d8b9c8' } },
  rest: { label: 'rest', style: { backgroundColor: '#171d29', borderColor: '#b7c3f0' } },
  stable: { label: 'stable', style: { backgroundColor: '#2a2131', borderColor: '#d8b9c8' } },
}

const prompts = [
  'What would help you land more steadily right now?',
  'Where can you rest without feeling like you need to keep moving?',
  'What grounds you when everything feels uncertain?',
  'How can this moment be a safe place to pause?',
]

export default function SteadyLandingBoard() {
  const [items, setItems] = useState<LandingItem[]>(starterItems)
  const [title, setTitle] = useState('')
  const [kind, setKind] = useState<LandingItem['kind']>('ground')
  ...
  const addItem = () => {
    ...
    setItems((current) => [...current, { id: `landing-${Date.now()}`, title: value, kind, done: false }])
    setTitle('')
  }
  ...
  return (
    <WellnessBoardShell title="Steady Landing" subtitle="Landing zone" badge="landing">
      <View style={styles.progressCard}>
        <View style={styles.progressHeader}>
          <Text style={styles.progressLabel}>Landing zone</Text>
      ...
      <View style={styles.list}>
        {items.map((item) => (
          <Pressable key={item.id} onPress={() => toggleItem(item.id)} style={[styles.item, item.done && styles.itemDone]}>
            ...
            <Text style={styles.itemState}>{item.done ? 'landed' : 'later'}</Text>
          </Pressable>
        ))}
      </View>
      ...
      <View style={styles.form}>
        <View style={styles.optionRow}>
          {(['ground', 'safe', 'rest', 'stable'] as LandingItem['kind'][]).map((option) => (
        ...
        <TextInput value={title} onChangeText={setTitle} placeholder="Add a grounding moment" placeholderTextColor="#8f8393" style={styles.input} />
