...
const kindMeta: Record<CheckInItem['kind'], { label: string; style: { backgroundColor: string; borderColor: string } }> = {
...
export default function LoveCheckInBoard() {
  ...
  const addItem = () => {
    ...
    setItems((current) => [...current, { id: `checkin-${Date.now()}`, text: value, kind, done: false }])
    setDraft('')
  }
  ...
  return (
    <WellnessBoardShell title="Love Check-In" subtitle="Check-in rhythm" badge="checkin">
      ...
      <View style={styles.form}>
        ...
        <TextInput value={draft} onChangeText={setDraft} placeholder="Share a need, offer, or ritual" placeholderTextColor="#8f8393" style={styles.input} />
      ...
      <View style={styles.featureCard}>
        ...
        <Text style={styles.featureText}>Check-ins work best when they are soft, honest, and kind. A small need is still a real one.</Text>
      </View>
    </WellnessBoardShell>
  )
}
