...
import { Heart, MessageCircleHeart, Plus, Sparkles, Wind } from 'lucide-react'
...
export default function LightEchoBoard() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('light-echo-board')
      if (raw) {
        const parsed = JSON.parse(raw)
        if (Array.isArray(parsed) && parsed.length) setItems(parsed)
      }
      const savedPrompt = localStorage.getItem('light-echo-prompt')
      if (savedPrompt) {
  ...
  useEffect(() => {
    localStorage.setItem('light-echo-board', JSON.stringify(items))
  }, [items])

  useEffect(() => {
    localStorage.setItem('light-echo-prompt', String(promptIndex))
  }, [promptIndex])
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-[var(--accent-2)]">
          ...
          <h3 className="font-dancing text-2xl">Light Echo</h3>
        </div>
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-gradient-to-r from-[var(--accent-2)] to-[var(--accent-1)] p-3 text-sm text-[var(--text-primary)]/80">
        ...
        <p>Sometimes the most helpful phrase is the one that returns with calm instead of force.</p>
      </div>
      ...
      <div className="flex items-center gap-2 rounded-2xl border border-[var(--accent-1)]/30 bg-[var(--bg-2)] p-3 text-sm text-[var(--accent-1)]">
        ...
        <span>Let the reassuring words stay light enough to land gently.</span>
      </div>
    </div>
  )
}
