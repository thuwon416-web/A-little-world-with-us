...
import { HeartHandshake, Moon, Plus, Sparkles, SunMedium } from 'lucide-react'
...
export default function GentleHoldBoard() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('gentle-hold-board')
      if (raw) {
        const parsed = JSON.parse(raw)
        if (Array.isArray(parsed) && parsed.length) setItems(parsed)
      }
      const savedPrompt = localStorage.getItem('gentle-hold-prompt')
      if (savedPrompt) {
  ...
  useEffect(() => {
    localStorage.setItem('gentle-hold-board', JSON.stringify(items))
  }, [items])

  useEffect(() => {
    localStorage.setItem('gentle-hold-prompt', String(promptIndex))
  }, [promptIndex])
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-[var(--accent-2)]">
          ...
          <h3 className="font-dancing text-2xl">Gentle Hold</h3>
        </div>
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-gradient-to-r from-[var(--accent-2)] to-[var(--accent-1)] p-3 text-sm text-[var(--text-primary)]/80">
        ...
        <p>
          To hold someone gently is not to carry every burden. It is to make room for comfort and
          for breath.
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 rounded-2xl border border-[var(--accent-1)]/20 bg-[var(--bg-2)] p-3 text-sm text-[var(--accent-1)]">
        ...
        <span>Support can be soft, warm, and steady all at once.</span>
      </div>
    </div>
  )
}
