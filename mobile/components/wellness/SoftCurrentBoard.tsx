...
type CurrentItem = { id: string; description: string; type: 'flow' | 'ease' | 'move' | 'drift'; done: boolean }

const starterItems: CurrentItem[] = [
  { id: 'current-1', description: 'Let today flow at its own gentle pace', type: 'flow', done: true },
  { id: 'current-2', description: 'Move with the current instead of against it', type: 'ease', done: false },
  { id: 'current-3', description: 'Trust the soft drift of what unfolds naturally', type: 'drift', done: true },
]

const typeMeta: Record<CurrentItem['type'], { label: string; style: { backgroundColor: string; borderColor: string } }> = {
...
export default function SoftCurrentBoard() {
  ...
  const addItem = () => {
    ...
    setItems((current) => [...current, { id: `current-${Date.now()}`, description: value, type, done: false }])
    setDescription('')
  }
        ...
        {items.map((item) => (
          <Pressable key={item.id} onPress={() => toggleItem(item.id)} style={[styles.item, item.done && styles.itemDone]}>
            <View style={styles.itemInner}>
              ...
              <Text style={[styles.itemText, item.done && styles.itemTextDone]}>{item.description}</Text>
            </View>
      ...
      <View style={styles.form}>
        <View style={styles.optionRow}>
          {(['flow', 'ease', 'move', 'drift'] as CurrentItem['type'][]).map((option) => (
            <Pressable key={option} onPress={() => setType(option)} style={[styles.option, option === type && styles.optionSelected]}>
              <Text style={[styles.optionText, option === type && styles.optionTextSelected]}>{option}</Text>
            </Pressable>
          ))}
        </View>
        <TextInput value={description} onChangeText={setDescription} placeholder="Add a flow moment" placeholderTextColor="#8f8393" style={styles.input} />
