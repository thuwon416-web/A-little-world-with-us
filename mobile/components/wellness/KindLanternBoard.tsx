...
type LanternItem = { id: string; title: string; glow: 'warm' | 'gentle' | 'bright' | 'steady'; done: boolean }

const starterItems: LanternItem[] = [
  { id: 'lantern-1', title: 'Notice the tiny light that helps us feel closer', glow: 'warm', done: true },
  { id: 'lantern-2', title: 'Keep the next step bright enough to feel safe', glow: 'gentle', done: false },
  { id: 'lantern-3', title: 'Let reassurance arrive as a steady glow, not a demand', glow: 'steady', done: true },
]

const glowMeta: Record<LanternItem['glow'], { label: string; style: { backgroundColor: string; borderColor: string } }> = {
...
const prompts = [
  'What gentle glow helps you both feel safe?',
  'Where is the light trying to arrive naturally?',
  'What warmth can you offer without dimming your own light?',
  'How can reassurance feel like comfort, not obligation?',
]
...
export default function KindLanternBoard() {
  ...
  const addItem = () => {
    ...
    if (!value) return
    setItems((current) => [...current, { id: `lantern-${Date.now()}`, title: value, glow, done: false }])
    setTitle('')
  }
  ...
  return (
    <WellnessBoardShell title="Kind Lantern" subtitle="Lantern glow" badge="lantern">
      <View style={styles.progressCard}>
        <View style={styles.progressHeader}>
          <Text style={styles.progressLabel}>Lantern glow</Text>
        ...
        {items.map((item) => (
          <Pressable key={item.id} onPress={() => toggleItem(item.id)} style={[styles.item, item.done && styles.itemDone]}>
            <View style={styles.itemInner}>
              <View style={[styles.pill, glowMeta[item.glow].style]}>
                <Text style={styles.pillText}>{glowMeta[item.glow].label}</Text>
              </View>
              <Text style={[styles.itemText, item.done && styles.itemTextDone]}>{item.title}</Text>
            </View>
            <Text style={styles.itemState}>{item.done ? 'glowing' : 'later'}</Text>
          </Pressable>
        ))}
      </View>
      ...
      <View style={styles.form}>
        <View style={styles.optionRow}>
          {(['warm', 'gentle', 'bright', 'steady'] as LanternItem['glow'][]).map((option) => (
            <Pressable key={option} onPress={() => setGlow(option)} style={[styles.option, option === glow && styles.optionSelected]}>
              <Text style={[styles.optionText, option === glow && styles.optionTextSelected]}>{option}</Text>
            </Pressable>
          ))}
        </View>
        <TextInput value={title} onChangeText={setTitle} placeholder="Add a light moment" placeholderTextColor="#8f8393" style={styles.input} />
