...
const MemoryCardComponent = function MemoryCard({ imageUrl, caption, date, index, priority = false, onOpen }: MemoryCardProps) {
  ...
  return <motion.button type="button" onClick={onOpen} initial={{ opacity: 0, y: 18 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.28, delay: index * 0.03 }} className={`glass-card group w-full overflow-hidden p-3 text-left ${rotation} hover:rotate-0 hover:glow-rose`}>
    <div className="relative aspect-[4/3] overflow-hidden rounded-xl bg-gradient-to-br from-accent-1/30 via-card to-accent-2/25">
    ...
    <div className="px-2 pb-1 pt-4"><h3 className="font-serif text-xl text-text-1">{caption || 'A moment together'}</h3><p className="mt-1 text-xs uppercase tracking-[0.14em] text-text-2">{new Date(date).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}</p></div>
  </motion.button>
}
