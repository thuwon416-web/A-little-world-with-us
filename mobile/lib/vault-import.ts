...
export async function importVault(
  ...
  try {
    ...
    throw new Error('The selected Vault export is not valid JSON.')
  }
  if (!parsed || typeof parsed !== 'object') throw new Error('Unsupported Vault export.')
  const payload = parsed as ImportPayload
  if (
    payload.version !== 1 ||
    payload.kdfParams?.algorithm !== 'PBKDF2' ||
    payload.kdfParams.iterations !== 100000 ||
    payload.kdfParams.hash !== 'SHA-256' ||
    !Array.isArray(payload.credentials) ||
    !payload.integrityHash
  )
    throw new Error('Unsupported or incomplete Vault export.')
  const { integrityHash, ...unsigned } = payload
  if ((await digest(JSON.stringify(unsigned))) !== integrityHash)
    throw new Error('Vault export integrity check failed.')
  ...
  for (const item of payload.credentials) {
    ...
    if (labels.has(item.label.trim().toLowerCase())) {
      result.skipped += 1
      continue
    }
    try {
      ...
      result.errors.push(
        `${item.label}: ${error instanceof Error ? error.message : 'Unable to import credential.'}`
      )
    }
  }
  return result
