...
type ConnectionItem = {
  id: string
  title: string
  kind: 'closeness' | 'reassure' | 'calm' | 'notice'
  done: boolean
}

const starterItems: ConnectionItem[] = [
  {
    id: 'connection-1',
    title: 'Sit a little closer and let the silence be enough',
    kind: 'closeness',
    done: true,
  },
  {
    id: 'connection-2',
    title: 'Say one reassuring thing without fixing anything',
    kind: 'reassure',
    done: false,
  },
  {
    id: 'connection-3',
    title: 'Notice what feels calmer now that you are here',
    kind: 'calm',
    done: true,
  },
]
...
const prompts = [
  'What makes you feel most held right now?',
  'What one sentence would make the connection feel safer?',
  'What part of being together feels calm and unforced?',
  'What quiet signal tells you love is still here?',
]

export default function SoftConnectionBoard() {
  const [items, setItems] = useState<ConnectionItem[]>(starterItems)
  const [title, setTitle] = useState('')
  const [kind, setKind] = useState<ConnectionItem['kind']>('closeness')
  const [promptIndex, setPromptIndex] = useState(0)

  useEffect(() => {
    try {
      const raw = localStorage.getItem('soft-connection-board')
      if (raw) {
        const parsed = JSON.parse(raw)
        if (Array.isArray(parsed) && parsed.length) setItems(parsed)
      }
      const savedPrompt = localStorage.getItem('soft-connection-prompt')
      if (savedPrompt) {
  ...
  useEffect(() => {
    localStorage.setItem('soft-connection-board', JSON.stringify(items))
  }, [items])

  useEffect(() => {
    localStorage.setItem('soft-connection-prompt', String(promptIndex))
  }, [promptIndex])
  ...
  const addItem = () => {
    ...
    setItems((current) => [
      ...current,
      { id: `soft-connection-${Date.now()}`, title: value, kind, done: false },
    ])
    setTitle('')
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-[var(--accent-2)]">
          ...
          <h3 className="font-dancing text-2xl">Soft Connection</h3>
        </div>
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-[var(--card-bg)]/15 p-3">
        <div className="mb-2 flex gap-2 overflow-x-auto">
          {(['closeness', 'reassure', 'calm', 'notice'] as ConnectionItem['kind'][]).map(
            (option) => (
              <button
                key={option}
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="Add a soft connection cue"
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-gradient-to-r from-[var(--accent-2)] to-[var(--accent-1)] p-3 text-sm text-[var(--text-primary)]/80">
        ...
        <p>
          Connection does not need to be dramatic to be deep. A soft rhythm is a real kind of
          closeness.
        </p>
      </div>
    </div>
