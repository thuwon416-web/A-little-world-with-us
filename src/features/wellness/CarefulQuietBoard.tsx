...
type QuietItem = {
  id: string
  title: string
  tone: 'quiet' | 'warmth' | 'ease' | 'calm'
  done: boolean
}

const starterItems: QuietItem[] = [
  { id: 'quiet-1', title: 'Let one pause be a kindness, not a problem', tone: 'quiet', done: true },
  { id: 'quiet-2', title: 'Use a soft phrase before a sharp truth', tone: 'warmth', done: false },
  {
    id: 'quiet-3',
    title: 'Notice the stillness before deciding what must change',
    tone: 'calm',
    done: true,
  },
]
...
const prompts = [
  'What makes the quiet feel safe instead of uncomfortable?',
  'Which soft phrase makes honesty feel more gentle?',
  'How can a pause become a doorway instead of a wall?',
  'What stillness helps both of you feel more connected than pressured?',
]

export default function CarefulQuietBoard() {
  const [items, setItems] = useState<QuietItem[]>(starterItems)
  const [title, setTitle] = useState('')
  const [tone, setTone] = useState<QuietItem['tone']>('quiet')
  const [promptIndex, setPromptIndex] = useState(0)

  useEffect(() => {
    try {
      const raw = localStorage.getItem('careful-quiet-board')
      if (raw) {
        const parsed = JSON.parse(raw)
        if (Array.isArray(parsed) && parsed.length) setItems(parsed)
      }
      const savedPrompt = localStorage.getItem('careful-quiet-prompt')
      if (savedPrompt) {
  ...
  useEffect(() => {
    localStorage.setItem('careful-quiet-board', JSON.stringify(items))
  }, [items])

  useEffect(() => {
    localStorage.setItem('careful-quiet-prompt', String(promptIndex))
  }, [promptIndex])
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-[var(--accent-2)]">
          ...
          <h3 className="font-dancing text-2xl">Careful Quiet</h3>
        </div>
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-[var(--card-bg)]/15 p-3">
        <div className="mb-2 flex gap-2 overflow-x-auto">
          {(['quiet', 'warmth', 'ease', 'calm'] as QuietItem['tone'][]).map((option) => (
            <button
              key={option}
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-gradient-to-r from-[var(--accent-2)] to-[var(--accent-1)] p-3 text-sm text-[var(--text-primary)]/80">
        ...
        <p>
          Careful quiet is not silence as hiding. It is the space that lets honesty and tenderness
      ...
      <div className="flex items-center gap-2 rounded-2xl border border-[var(--accent-1)]/30 bg-[var(--bg-2)] p-3 text-sm text-[var(--accent-1)]">
        ...
        <span>Let pause be a form of care, not a delay.</span>
      </div>
    </div>
  )
}
