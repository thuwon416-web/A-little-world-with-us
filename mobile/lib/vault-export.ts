...
async function digest(value: string): Promise<string> {
  const bytes = await Crypto.digestStringAsync(Crypto.CryptoDigestAlgorithm.SHA256, value, {
...
export async function exportVault(
  ...
  return JSON.stringify(
    { ...payload, integrityHash: await digest(JSON.stringify(payload)) },
    null,
    2
  )
}
