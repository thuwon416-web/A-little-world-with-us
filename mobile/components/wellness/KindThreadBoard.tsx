...
type ThreadItem = { id: string; title: string; tone: 'gentle' | 'warm' | 'bright' | 'steady'; done: boolean }

const starterItems: ThreadItem[] = [
  { id: 'thread-1', title: 'Notice one thread of care that keeps returning', tone: 'gentle', done: true },
  { id: 'thread-2', title: 'Weave one reassuring detail into the next interaction', tone: 'warm', done: false },
  { id: 'thread-3', title: 'Keep the pattern gentle enough to be easy to return to', tone: 'steady', done: true },
]

const toneMeta: Record<ThreadItem['tone'], { label: string; style: { backgroundColor: string; borderColor: string } }> = {
...
export default function KindThreadBoard() {
  ...
  const addItem = () => {
    ...
    if (!value) return
    setItems((current) => [...current, { id: `thread-${Date.now()}`, title: value, tone, done: false }])
    setTitle('')
  }
  ...
  return (
    <WellnessBoardShell title="Kind Thread" subtitle="Thread pattern" badge="thread">
      ...
      <View style={styles.form}>
        ...
        <TextInput value={title} onChangeText={setTitle} placeholder="Add a thread detail" placeholderTextColor="#8f8393" style={styles.input} />
