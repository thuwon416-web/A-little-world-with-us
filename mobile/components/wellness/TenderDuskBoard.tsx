...
type DuskItem = { id: string; feeling: string; mood: 'glow' | 'calm' | 'soft' | 'ease'; done: boolean }

const starterItems: DuskItem[] = [
  { id: 'dusk-1', feeling: 'Let the day settle into gentle rest', mood: 'glow', done: true },
  { id: 'dusk-2', feeling: 'Trade the busy energy for something more tender', mood: 'calm', done: false },
  { id: 'dusk-3', feeling: 'Make room for the quiet that wants to arrive', mood: 'soft', done: true },
]

const moodMeta: Record<DuskItem['mood'], { label: string; style: { backgroundColor: string; borderColor: string } }> = {
...
export default function TenderDuskBoard() {
  ...
  const addItem = () => {
    ...
    setItems((current) => [...current, { id: `dusk-${Date.now()}`, feeling: value, mood, done: false }])
    setFeeling('')
  }
  ...
  return (
    <WellnessBoardShell title="Tender Dusk" subtitle="Evening transition" badge="dusk">
      ...
      <View style={styles.form}>
        ...
        <TextInput value={feeling} onChangeText={setFeeling} placeholder="Add an evening transition" placeholderTextColor="#8f8393" style={styles.input} />
