...
import type { TimeCapsule } from '@/shared-types'

type CapsuleContext = Awaited<ReturnType<typeof getContext>>

export default function TimeCapsulesSection() {
  const { colors } = useTheme()
  const [items, setItems] = useState<TimeCapsule[]>([])
  const [context, setContext] = useState<CapsuleContext | null>(null)
  ...
  const add = async () => {
    ...
    if (!context?.link) return Alert.alert('Link a partner', 'A capsule needs a recipient.')
    const recipient =
