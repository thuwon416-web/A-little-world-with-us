...
const starterItems: RepairItem[] = [
  ...
  { id: 'repair-2', title: 'Use one true sentence: I feel hurt and I still love you', mode: 'repair', done: false },
  { id: 'repair-3', title: 'Offer a small physical comfort', mode: 'comfort', done: true },
]

const modeMeta: Record<RepairItem['mode'], { label: string; backgroundColor: string; borderColor: string; color: string }> = {
  pause: { label: 'pause', backgroundColor: '#201d29', borderColor: '#d8b9c8', color: '#f0c9d9' },
  repair: { label: 'repair', backgroundColor: '#201d29', borderColor: '#d8b9c8', color: '#f0c9d9' },
  comfort: { label: 'comfort', backgroundColor: '#2b2a22', borderColor: '#f3d59c', color: '#f5eacf' },
  renew: { label: 'renew', backgroundColor: '#1b2129', borderColor: '#b7d7c8', color: '#edf8f2' },
}
...
export default function SoftRepairBoard() {
  ...
  const addItem = () => {
    ...
    if (!value) return
    setItems((current) => [...current, { id: `soft-repair-${Date.now()}`, title: value, mode, done: false }])
    setTitle('')
  }
  ...
  return (
    <WellnessBoardShell title="Soft Repair" subtitle="Repair rhythm" badge={`${doneCount}/${items.length}`}>
      <View style={styles.progressBox}>
        <View style={styles.progressHeader}>
          <Text style={styles.progressLabel}>Repair rhythm</Text>
      ...
      <View style={styles.form}>
        <View style={styles.toneRow}>
          {(['pause', 'repair', 'comfort', 'renew'] as RepairItem['mode'][]).map((option) => (
            <Pressable key={option} onPress={() => setMode(option)} style={[styles.modeButton, mode === option && styles.modeButtonActive]}>
              <Text style={[styles.modeText, mode === option && styles.modeTextActive]}>{option}</Text>
            </Pressable>
          ))}
        </View>

        <TextInput value={title} onChangeText={setTitle} placeholder="Add a gentle repair action" placeholderTextColor="#8f8393" style={styles.input} />
      ...
      <View style={styles.promptCard}>
        <View style={styles.promptHeader}>
          <Text style={styles.promptLabel}>Repair prompt</Text>
