...
import { Home, Moon, Plus, Sparkles, Wind } from 'lucide-react'

type ArrivalItem = {
  id: string
  title: string
  tone: 'arrival' | 'warmth' | 'ease' | 'ground'
  done: boolean
}

const starterItems: ArrivalItem[] = [
  { id: 'arrival-1', title: 'Unclench before we begin the evening', tone: 'ground', done: true },
  {
    id: 'arrival-2',
    title: 'Give the other person one soft welcome cue',
    tone: 'warmth',
    done: false,
  },
  { id: 'arrival-3', title: 'Let the room settle before the next task', tone: 'ease', done: true },
]
...
const prompts = [
  'What helps the transition from the day into being together feel softer?',
  'Which tiny welcome makes the other person feel more at home?',
  'What signal tells both of you, “We can land here without effort”?',
  'What small sensory cue makes the moment feel calmer and kinder?',
]

export default function SlowArrivalBoard() {
  const [items, setItems] = useState<ArrivalItem[]>(starterItems)
  const [title, setTitle] = useState('')
  const [tone, setTone] = useState<ArrivalItem['tone']>('arrival')
  const [promptIndex, setPromptIndex] = useState(0)

  useEffect(() => {
    try {
      const raw = localStorage.getItem('slow-arrival-board')
      if (raw) {
        const parsed = JSON.parse(raw)
        if (Array.isArray(parsed) && parsed.length) setItems(parsed)
      }
      const savedPrompt = localStorage.getItem('slow-arrival-prompt')
      if (savedPrompt) {
  ...
  useEffect(() => {
    localStorage.setItem('slow-arrival-board', JSON.stringify(items))
  }, [items])

  useEffect(() => {
    localStorage.setItem('slow-arrival-prompt', String(promptIndex))
  }, [promptIndex])
  ...
  const addItem = () => {
    ...
    setItems((current) => [
      ...current,
      { id: `arrival-${Date.now()}`, title: value, tone, done: false },
    ])
    setTitle('')
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-[var(--accent-2)]">
          ...
          <h3 className="font-dancing text-2xl">Slow Arrival</h3>
        </div>
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-[var(--card-bg)]/15 p-3">
        <div className="mb-2 flex gap-2 overflow-x-auto">
          {(['arrival', 'warmth', 'ease', 'ground'] as ArrivalItem['tone'][]).map((option) => (
            <button
              key={option}
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="Add a soft arrival cue"
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-gradient-to-r from-[var(--accent-1)] to-[var(--accent-1)] p-3 text-sm text-[var(--text-primary)]/80">
        ...
        <p>Home is often not a place but the way we soften when we arrive back to each other.</p>
      </div>
      ...
      <div className="flex items-center gap-2 rounded-2xl border border-[var(--accent-1)]/20 bg-[var(--bg-2)] p-3 text-sm text-[var(--accent-1)]">
        ...
        <span>Ease into the evening with kindness, not urgency.</span>
      </div>
    </div>
  )
}
