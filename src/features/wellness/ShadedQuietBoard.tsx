...
type ShadedItem = {
  id: string
  title: string
  tone: 'shade' | 'warmth' | 'ease' | 'calm'
  done: boolean
}

const starterItems: ShadedItem[] = [
  {
    id: 'shaded-1',
    title: 'Let the room dim into softness before the next decision',
    tone: 'shade',
    done: true,
  },
  {
    id: 'shaded-2',
    title: 'Choose one gentler phrase for the next exchange',
    tone: 'warmth',
    done: false,
  },
  { id: 'shaded-3', title: 'Pause long enough for tenderness to arrive', tone: 'calm', done: true },
]
...
const prompts = [
  'What softer shade does the moment need to feel less sharp?',
  'Which pause lets the room feel gentler before the next conversation?',
  'How can we rest in tenderness without losing honesty?',
  'What quiet cue brings both of you back to calm more easily?',
]

export default function ShadedQuietBoard() {
  const [items, setItems] = useState<ShadedItem[]>(starterItems)
  const [title, setTitle] = useState('')
  const [tone, setTone] = useState<ShadedItem['tone']>('shade')
  const [promptIndex, setPromptIndex] = useState(0)

  useEffect(() => {
    try {
      const raw = localStorage.getItem('shaded-quiet-board')
      if (raw) {
        const parsed = JSON.parse(raw)
        if (Array.isArray(parsed) && parsed.length) setItems(parsed)
      }
      const savedPrompt = localStorage.getItem('shaded-quiet-prompt')
      if (savedPrompt) {
  ...
  useEffect(() => {
    localStorage.setItem('shaded-quiet-board', JSON.stringify(items))
  }, [items])

  useEffect(() => {
    localStorage.setItem('shaded-quiet-prompt', String(promptIndex))
  }, [promptIndex])
  ...
  const addItem = () => {
    ...
    setItems((current) => [
      ...current,
      { id: `shaded-${Date.now()}`, title: value, tone, done: false },
    ])
    setTitle('')
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-[var(--accent-2)]">
          ...
          <h3 className="font-dancing text-2xl">Shaded Quiet</h3>
        </div>
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-[var(--card-bg)]/15 p-3">
        <div className="mb-2 flex gap-2 overflow-x-auto">
          {(['shade', 'warmth', 'ease', 'calm'] as ShadedItem['tone'][]).map((option) => (
            <button
              key={option}
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="Add a shaded quiet cue"
          ...
          <button
            ...
            aria-label="Add shaded quiet cue"
          >
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-gradient-to-r from-[var(--accent-2)] to-[var(--accent-1)] p-3 text-sm text-[var(--text-primary)]/80">
        ...
        <p>
          Sometimes a quieter shade is the only thing that lets the heart feel safe enough to
          breathe.
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 rounded-2xl border border-[var(--accent-1)]/30 bg-[var(--bg-2)] p-3 text-sm text-[var(--accent-1)]">
        ...
        <span>Let stillness be a shelter, not a void.</span>
      </div>
    </div>
  )
}
