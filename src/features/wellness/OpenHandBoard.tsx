...
import { Hand, Heart, Plus, Sparkles, Wind } from 'lucide-react'

type HandItem = {
  id: string
  title: string
  tone: 'open' | 'warmth' | 'ease' | 'calm'
  done: boolean
}
...
const starterItems: HandItem[] = [
  ...
  {
    id: 'hand-2',
    title: 'Choose one kindness cue to reach across the day',
    tone: 'warmth',
    done: false,
  },
  {
    id: 'hand-3',
    title: 'Leave room for softness before urgency returns',
    tone: 'calm',
    done: true,
  },
]
...
export default function OpenHandBoard() {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem('open-hand-board')
      if (raw) {
        const parsed = JSON.parse(raw)
        if (Array.isArray(parsed) && parsed.length) setItems(parsed)
      }
      const savedPrompt = localStorage.getItem('open-hand-prompt')
      if (savedPrompt) {
  ...
  useEffect(() => {
    localStorage.setItem('open-hand-board', JSON.stringify(items))
  }, [items])

  useEffect(() => {
    localStorage.setItem('open-hand-prompt', String(promptIndex))
  }, [promptIndex])
  ...
  const addItem = () => {
    ...
    setItems((current) => [
      ...current,
      { id: `hand-${Date.now()}`, title: value, tone, done: false },
    ])
    setTitle('')
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-[var(--accent-2)]">
          ...
          <h3 className="font-dancing text-2xl">Open Hand</h3>
        </div>
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-gradient-to-r from-[var(--accent-2)] to-[var(--accent-1)] p-3 text-sm text-[var(--text-primary)]/80">
        ...
        <p>
          Open hands are quiet acts of care: not trying to solve everything, just making it easier
          to be held.
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 rounded-2xl border border-[var(--accent-1)]/30 bg-[var(--bg-2)] p-3 text-sm text-[var(--accent-1)]">
        ...
        <span>Offer help without adding pressure.</span>
      </div>
    </div>
  )
}
