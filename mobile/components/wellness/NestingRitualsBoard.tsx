...
type NestItem = { id: string; title: string; kind: 'soften' | 'nest' | 'slow' | 'warm'; done: boolean }

const starterItems: NestItem[] = [
  { id: 'nest-1', title: 'Dim the lights and let the room feel slower', kind: 'soften', done: true },
  { id: 'nest-2', title: 'Make a tiny corner feel like a shared haven', kind: 'nest', done: false },
  { id: 'nest-3', title: 'Finish the evening without rushing the moment', kind: 'slow', done: true },
]

const kindMeta: Record<NestItem['kind'], { label: string; style: { backgroundColor: string; borderColor: string } }> = {
  soften: { label: 'soften', style: { backgroundColor: '#2a2131', borderColor: '#d8b9c8' } },
  nest: { label: 'nest', style: { backgroundColor: '#2d2234', borderColor: '#d8b9c8' } },
  slow: { label: 'slow', style: { backgroundColor: '#171d29', borderColor: '#b7c3f0' } },
  warm: { label: 'warm', style: { backgroundColor: '#1c2129', borderColor: '#b0d8c5' } },
}

const prompts = [
  'What tiny room change would make the home feel softer?',
  'Where can you build your shared haven tonight?',
  'How can you slow down the ending of the day together?',
  'What warmth wants to arrive in the space between you?',
]

export default function NestingRitualsBoard() {
  const [items, setItems] = useState<NestItem[]>(starterItems)
  const [title, setTitle] = useState('')
  const [kind, setKind] = useState<NestItem['kind']>('nest')
  ...
  const addItem = () => {
    ...
    if (!value) return
    setItems((current) => [...current, { id: `nest-${Date.now()}`, title: value, kind, done: false }])
    setTitle('')
  }
  ...
  return (
    <WellnessBoardShell title="Nesting Rituals" subtitle="Nest log" badge="nesting">
      <View style={styles.progressCard}>
        <View style={styles.progressHeader}>
          <Text style={styles.progressLabel}>Nest log</Text>
      ...
      <View style={styles.list}>
        {items.map((item) => (
          <Pressable key={item.id} onPress={() => toggleItem(item.id)} style={[styles.item, item.done && styles.itemDone]}>
            ...
            <Text style={styles.itemState}>{item.done ? 'nested' : 'later'}</Text>
          </Pressable>
        ))}
      </View>
      ...
      <View style={styles.form}>
        ...
        <TextInput value={title} onChangeText={setTitle} placeholder="Add a nesting detail" placeholderTextColor="#8f8393" style={styles.input} />
