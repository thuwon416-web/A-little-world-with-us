...
type ResetItem = {
  id: string
  title: string
  tone: 'reset' | 'warmth' | 'ease' | 'ground'
  done: boolean
}

const starterItems: ResetItem[] = [
  {
    id: 'warm-reset-1',
    title: 'Take a full beat before the next conversation',
    tone: 'ground',
    done: true,
  },
  {
    id: 'warm-reset-2',
    title: 'Offer one tiny comfort instead of a long explanation',
    tone: 'warmth',
    done: false,
  },
  {
    id: 'warm-reset-3',
    title: 'Let the space feel a little softer before moving on',
    tone: 'ease',
    done: true,
  },
]
...
const prompts = [
  'What tiny reset helps the room feel softer again?',
  'Which small action makes the next step feel less heavy?',
  'What gentle cue lets us return to warmth without performance?',
  'How can we reset without turning the moment into a task?',
]

export default function WarmResetBoard() {
  const [items, setItems] = useState<ResetItem[]>(starterItems)
  const [title, setTitle] = useState('')
  const [tone, setTone] = useState<ResetItem['tone']>('reset')
  const [promptIndex, setPromptIndex] = useState(0)

  useEffect(() => {
    try {
      const raw = localStorage.getItem('warm-reset-board')
      if (raw) {
        const parsed = JSON.parse(raw)
        if (Array.isArray(parsed) && parsed.length) setItems(parsed)
      }
      const savedPrompt = localStorage.getItem('warm-reset-prompt')
      if (savedPrompt) {
  ...
  useEffect(() => {
    localStorage.setItem('warm-reset-board', JSON.stringify(items))
  }, [items])

  useEffect(() => {
    localStorage.setItem('warm-reset-prompt', String(promptIndex))
  }, [promptIndex])
  ...
  const addItem = () => {
    ...
    setItems((current) => [
      ...current,
      { id: `warm-reset-${Date.now()}`, title: value, tone, done: false },
    ])
    setTitle('')
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-[var(--accent-2)]">
          ...
          <h3 className="font-dancing text-2xl">Warm Reset</h3>
        </div>
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-[var(--card-bg)]/15 p-3">
        <div className="mb-2 flex gap-2 overflow-x-auto">
          {(['reset', 'warmth', 'ease', 'ground'] as ResetItem['tone'][]).map((option) => (
            <button
              key={option}
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-gradient-to-r from-[var(--accent-2)] to-[var(--accent-1)] p-3">
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/30 bg-gradient-to-r from-[var(--accent-1)] to-[var(--accent-1)] p-3 text-sm text-[var(--text-primary)]/80">
        ...
        <p>Warm resets do not erase the day—they create a gentler route back to one another.</p>
      </div>
      ...
      <div className="flex items-center gap-2 rounded-2xl border border-[var(--accent-1)]/20 bg-[var(--bg-2)] p-3 text-sm text-[var(--accent-1)]">
        ...
        <span>Return with softness before you return to urgency.</span>
      </div>
    </div>
  )
}
