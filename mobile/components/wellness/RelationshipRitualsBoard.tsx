...
type RitualItem = { id: string; title: string; kind: 'together' | 'care' | 'bond' | 'safe'; done: boolean }

const starterItems: RitualItem[] = [
  { id: 'ritual-1', title: 'Set a ritual that deepens our bond', kind: 'bond', done: true },
  { id: 'ritual-2', title: 'Let care arrive without being announced', kind: 'care', done: false },
  { id: 'ritual-3', title: 'Build a pattern we both feel safe returning to', kind: 'safe', done: true },
]

const kindMeta: Record<RitualItem['kind'], { label: string; style: { backgroundColor: string; borderColor: string } }> = {
  together: { label: 'together', style: { backgroundColor: '#2d2234', borderColor: '#d8b9c8' } },
  care: { label: 'care', style: { backgroundColor: '#171d29', borderColor: '#b7c3f0' } },
  bond: { label: 'bond', style: { backgroundColor: '#1c2129', borderColor: '#b0d8c5' } },
  safe: { label: 'safe', style: { backgroundColor: '#2a2131', borderColor: '#d8b9c8' } },
}

const prompts = [
  'What ritual brings you closer together?',
  'Where does care feel most natural?',
  'What bond wants to deepen?',
  'How can this pattern feel safer?',
]

export default function RelationshipRitualsBoard() {
  const [items, setItems] = useState<RitualItem[]>(starterItems)
  const [title, setTitle] = useState('')
  const [kind, setKind] = useState<RitualItem['kind']>('together')
  ...
  const addItem = () => {
    ...
    if (!value) return
    setItems((current) => [...current, { id: `ritual-${Date.now()}`, title: value, kind, done: false }])
    setTitle('')
  }
  ...
  return (
    <WellnessBoardShell title="Relationship Rituals" subtitle="Ritual keeper" badge="ritual">
      <View style={styles.progressCard}>
        <View style={styles.progressHeader}>
          <Text style={styles.progressLabel}>Ritual keeper</Text>
      ...
      <View style={styles.list}>
        {items.map((item) => (
          <Pressable key={item.id} onPress={() => toggleItem(item.id)} style={[styles.item, item.done && styles.itemDone]}>
            ...
            <Text style={styles.itemState}>{item.done ? 'kept' : 'later'}</Text>
          </Pressable>
        ))}
      </View>
      ...
      <View style={styles.form}>
        <View style={styles.optionRow}>
          {(['together', 'care', 'bond', 'safe'] as RitualItem['kind'][]).map((option) => (
            <Pressable key={option} onPress={() => setKind(option)} style={[styles.option, option === kind && styles.optionSelected]}>
              <Text style={[styles.optionText, option === kind && styles.optionTextSelected]}>{option}</Text>
            </Pressable>
          ))}
        </View>
        <TextInput value={title} onChangeText={setTitle} placeholder="Add a relationship ritual" placeholderTextColor="#8f8393" style={styles.input} />
