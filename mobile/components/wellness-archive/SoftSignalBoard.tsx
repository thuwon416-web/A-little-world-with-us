...
type SignalItem = {
  id: string
  message: string
  signal: 'subtle' | 'presence' | 'tone' | 'steady'
  done: boolean
}
...
const starterItems: SignalItem[] = [
  {
    id: 'softsignal-1',
    message: 'A subtle shift that changes how the day feels',
    signal: 'subtle',
    done: true,
  },
  {
    id: 'softsignal-2',
    message: 'A steady presence that reminds us we are not alone',
    signal: 'presence',
    done: false,
  },
  {
    id: 'softsignal-3',
    message: 'A gentle tone that softens what might otherwise feel rough',
    signal: 'tone',
    done: true,
  },
]
...
export default function SoftSignalBoard() {
  ...
  const addItem = () => {
    ...
    setItems((current) => [
      ...current,
      { id: `softsignal-${Date.now()}`, message: value, signal, done: false },
    ])
    setMessage('')
  }
