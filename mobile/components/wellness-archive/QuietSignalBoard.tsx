...
type SignalItem = {
  id: string
  message: string
  signal: 'whisper' | 'gentle' | 'soft' | 'still'
  done: boolean
}
...
const starterItems: SignalItem[] = [
  {
    id: 'signal-1',
    message: 'A soft whisper that says we are still here for each other',
    signal: 'whisper',
    done: true,
  },
  {
    id: 'signal-2',
    message: 'A gentle reminder that small gestures count as care',
    signal: 'gentle',
    done: false,
  },
  {
    id: 'signal-3',
    message: 'A quiet signal of presence without needing grand gestures',
    signal: 'still',
    done: true,
  },
]
...
export default function QuietSignalBoard() {
  ...
  const addItem = () => {
    ...
    setItems((current) => [
      ...current,
      { id: `signal-${Date.now()}`, message: value, signal, done: false },
    ])
    setMessage('')
  }
    ...
    <WellnessBoardShell title="Quiet Signal" subtitle="Signal count" badge="signal">
      ...
      <View style={styles.form}>
        <View style={styles.optionRow}>
          {(['whisper', 'gentle', 'soft', 'still'] as SignalItem['signal'][]).map((option) => (
            <Pressable
              key={option}
              onPress={() => setSignal(option)}
              style={[styles.option, option === signal && styles.optionSelected]}
            >
              <Text style={[styles.optionText, option === signal && styles.optionTextSelected]}>
                {option}
              </Text>
            </Pressable>
          ))}
      ...
      <View style={styles.featureCard}>
        ...
        <Text style={styles.featureText}>
          A whisper is enough when it comes from the heart. Small signals count as presence.
        </Text>
      </View>
    </WellnessBoardShell>
  )
