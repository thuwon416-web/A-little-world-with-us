...
const starterItems: DuskItem[] = [
  {
    id: 'dusk-1',
    title: 'Let the evening arrive before the next demand',
    tone: 'dusk',
    done: true,
  },
  {
    id: 'dusk-2',
    title: 'Choose one tender cue for the closing hour',
    tone: 'warmth',
    done: false,
  },
  {
    id: 'dusk-3',
    title: 'Keep the final stretch gentle enough to soften into rest',
    tone: 'calm',
    done: true,
  },
]
...
const prompts = [
  'What makes the evening feel like a gentle return instead of a hard stop?',
  'Which small cue brings calm as the day begins to dim?',
  'How can we close the night with tenderness instead of urgency?',
  'What soft ritual helps the heart settle before sleep?',
]

export default function TenderDuskBoard() {
  const [items, setItems] = useState<DuskItem[]>(starterItems)
  const [title, setTitle] = useState('')
  const [tone, setTone] = useState<DuskItem['tone']>('dusk')
  const [promptIndex, setPromptIndex] = useState(0)

  useEffect(() => {
    try {
      const raw = localStorage.getItem('tender-dusk-board')
      if (raw) {
        const parsed = JSON.parse(raw)
        if (Array.isArray(parsed) && parsed.length) setItems(parsed)
      }
      const savedPrompt = localStorage.getItem('tender-dusk-prompt')
      if (savedPrompt) {
  ...
  useEffect(() => {
    localStorage.setItem('tender-dusk-board', JSON.stringify(items))
  }, [items])

  useEffect(() => {
    localStorage.setItem('tender-dusk-prompt', String(promptIndex))
  }, [promptIndex])
  ...
  const addItem = () => {
    ...
    setItems((current) => [
      ...current,
      { id: `dusk-${Date.now()}`, title: value, tone, done: false },
    ])
    setTitle('')
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-[var(--accent-2)]">
          ...
          <h3 className="font-dancing text-2xl">Tender Dusk</h3>
        </div>
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-[var(--card-bg)]/15 p-3">
        <div className="mb-2 flex gap-2 overflow-x-auto">
          {(['dusk', 'warmth', 'ease', 'calm'] as DuskItem['tone'][]).map((option) => (
            <button
              key={option}
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-gradient-to-r from-[var(--accent-2)] to-[var(--accent-1)] p-3 text-sm text-[var(--text-primary)]/80">
        ...
        <p>Evening is not a signal to shut down. It is a chance to let the day soften into care.</p>
      </div>
      ...
      <div className="flex items-center gap-2 rounded-2xl border border-[var(--accent-1)]/30 bg-[var(--bg-2)] p-3 text-sm text-[var(--accent-1)]">
        ...
        <span>Let the night arrive gently, and carry plenty of warmth with it.</span>
      </div>
    </div>
  )
}
