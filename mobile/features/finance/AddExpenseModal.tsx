...
export default function AddExpenseModal({
  ...
  const save = async () => {
    ...
    if (
      splitType === 'custom' &&
      (!Number.isFinite(exactYou) ||
        !Number.isFinite(exactPartner) ||
        Math.abs(exactYou + exactPartner - parsedAmount) > 0.01)
    )
      return setError('Shares must add up to the total amount.')
    ...
    <Modal visible={visible} onClose={onClose} title="Add expense">
      ...
      <View style={styles.pillRow}>
        {EXPENSE_CATEGORIES.map((item) => (
          <TouchableOpacity
            ...
            style={[styles.pill, category === item.value && styles.activePill]}
          >
            <Text style={category === item.value ? styles.activeText : styles.pillText}>
              {item.label}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      <View style={styles.pillRow}>
        <TouchableOpacity
          ...
          style={[styles.pill, paidBy === currentUserId && styles.activePill]}
        >
          <Text style={paidBy === currentUserId ? styles.activeText : styles.pillText}>You</Text>
        </TouchableOpacity>
        <TouchableOpacity
          onPress={() => partnerId && setPaidBy(partnerId)}
          style={[styles.pill, paidBy === partnerId && styles.activePill]}
        >
          <Text style={paidBy === partnerId ? styles.activeText : styles.pillText}>Partner</Text>
        </TouchableOpacity>
      </View>
      ...
      <View style={styles.pillRow}>
        {SPLIT_TYPES.map((item) => (
          <TouchableOpacity
            key={item.value}
            onPress={() => setSplitType(item.value)}
            style={[styles.pill, splitType === item.value && styles.activePill]}
          >
            <Text style={splitType === item.value ? styles.activeText : styles.pillText}>
              {item.label}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      {splitType === 'percentage' ? (
        <>
          <TextInput
            ...
            style={styles.input}
          />
          <Text style={styles.hint}>Your share: {100 - Number(partnerPercentage || 0)}%</Text>
        </>
      ) : null}
      {splitType === 'custom' ? (
      ...
      <View style={styles.actions}>
        ...
        <TouchableOpacity onPress={() => void save()} disabled={submitting} style={styles.save}>
