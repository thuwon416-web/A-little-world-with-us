...
export default function CareScreen() {
  ...
  const saveCheckIn = async (periodStarted = false) => {
    try {
      setSaving(true)
      await saveTodayCareLog({ mood: mood || undefined, symptoms: selectedSymptoms, sex, discharge, digestion, pregnancyTest, contraceptives, activities: selectedActivities, waterIntake: waterIntake ? Number(waterIntake) : undefined, weight: weight ? Number(weight) : undefined, basalTemp: basalTemp ? Number(basalTemp) : undefined, notes: notes || undefined, periodStarted })
  ...
  return <ScrollView style={styles.screen} contentContainerStyle={styles.content}>
    ...
    <View style={styles.hero}><Text style={styles.heroLabel}>SHARED CYCLE</Text><Text style={styles.days}>Log a period</Text><Text style={styles.heroNote}>A forecast appears after period history is recorded.</Text><Text style={styles.disclaimer}>Predictions are estimates, not medical advice.</Text></View>
    ...
    <View style={styles.card}><Text style={styles.cardTitle}>Discharge</Text><Chips options={dischargeOptions} selected={discharge} onToggle={(value) => toggle(discharge, value, setDischarge)} tone="purple" /></View>
    <View style={styles.card}><Text style={styles.cardTitle}>Digestion & stool</Text><Chips options={digestionOptions} selected={digestion} onToggle={(value) => toggle(digestion, value, setDigestion)} tone="purple" /></View>
    <View style={styles.card}><Text style={styles.cardTitle}>Pregnancy test</Text><Chips options={pregnancyTestOptions} selected={pregnancyTest} onToggle={(value) => toggle(pregnancyTest, value, setPregnancyTest)} /></View>
    <View style={styles.card}><Text style={styles.cardTitle}>Contraceptives</Text><Chips options={contraceptiveOptions} selected={contraceptives} onToggle={(value) => toggle(contraceptives, value, setContraceptives)} tone="green" /></View>
    ...
    <View style={styles.card}><Text style={styles.cardTitle}>Daily details</Text><View style={styles.inputs}><TextInput value={waterIntake} onChangeText={setWaterIntake} placeholder="Water (ml)" placeholderTextColor="#bcaed1" keyboardType="numeric" style={styles.input} /><TextInput value={weight} onChangeText={setWeight} placeholder="Weight (kg)" placeholderTextColor="#bcaed1" keyboardType="decimal-pad" style={styles.input} /><TextInput value={basalTemp} onChangeText={setBasalTemp} placeholder="Basal temp (°C)" placeholderTextColor="#bcaed1" keyboardType="decimal-pad" style={styles.input} /></View><TextInput value={notes} onChangeText={setNotes} placeholder="Notes for today" placeholderTextColor="#bcaed1" multiline style={[styles.input, styles.notes]} /></View>
...
const styles = StyleSheet.create({
  screen: { flex: 1, backgroundColor: '#1A0B2E' }, content: { padding: 20, paddingTop: 64, paddingBottom: 36 }, eyebrow: { color: '#c9b9dd', fontWeight: '700', letterSpacing: 1.3, fontSize: 10 }, title: { color: '#fff8ff', fontWeight: '800', fontSize: 30, marginTop: 6 }, hero: { marginTop: 18, borderRadius: 28, padding: 28, alignItems: 'center', backgroundColor: '#2d1b4e', borderWidth: 1, borderColor: '#ffffff20' }, heroLabel: { color: '#c9b9dd', fontSize: 11, fontWeight: '800', letterSpacing: 1.5 }, days: { color: '#FFD700', fontWeight: '800', fontSize: 38, marginTop: 8 }, heroNote: { color: '#f7eaf4', fontSize: 15, fontWeight: '600', marginTop: 12, textAlign: 'center' }, disclaimer: { color: '#c9b9dd', fontSize: 11, marginTop: 8, textAlign: 'center' }, actions: { flexDirection: 'row', justifyContent: 'space-around', marginVertical: 24 }, action: { alignItems: 'center', width: 92 }, actionIcon: { width: 58, height: 58, alignItems: 'center', justifyContent: 'center', borderRadius: 29, backgroundColor: '#32184f', borderWidth: 1, borderColor: '#ff6b9d80' }, actionEmoji: { fontSize: 25 }, actionLabel: { color: '#f7eaf4', fontSize: 12, fontWeight: '600', marginTop: 9, textAlign: 'center' }, card: { backgroundColor: '#2d1b4e', borderRadius: 22, padding: 18, marginBottom: 16, borderWidth: 1, borderColor: '#ffffff1a' }, cardTitle: { color: '#fff8ff', fontSize: 19, fontWeight: '800' }, muted: { color: '#c9b9dd', fontSize: 13, marginTop: 5 }, chips: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 14 }, chip: { borderRadius: 20, paddingHorizontal: 13, paddingVertical: 9, backgroundColor: '#40245f' }, purpleChip: { backgroundColor: '#39235a' }, greenChip: { backgroundColor: '#184c45' }, chipSelected: { backgroundColor: '#ff5d89' }, chipText: { color: '#f0e7f7', fontWeight: '600', fontSize: 13 }, chipTextSelected: { color: '#fff' }, inputs: { gap: 10, marginTop: 14 }, input: { borderRadius: 12, borderWidth: 1, borderColor: '#ffffff24', backgroundColor: '#1f1037', color: '#fff8ff', paddingHorizontal: 12, paddingVertical: 11 }, notes: { marginTop: 10, minHeight: 82, textAlignVertical: 'top' }, saveButton: { marginTop: 4, alignItems: 'center', borderRadius: 16, backgroundColor: '#ff5d89', paddingVertical: 15 }, saveButtonDisabled: { opacity: .6 }, saveText: { color: '#fff', fontWeight: '800' },
})
