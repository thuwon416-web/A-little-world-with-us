...
type EchoItem = { id: string; title: string; tone: 'echo' | 'warmth' | 'ease' | 'calm'; done: boolean }

const starterItems: EchoItem[] = [
  { id: 'echo-1', title: 'Let one tender thought return more often than the stress', tone: 'echo', done: true },
  { id: 'echo-2', title: 'Give the day one gentle voice of reassurance', tone: 'warmth', done: false },
  { id: 'echo-3', title: 'Keep the inner tone softer than the pressure', tone: 'calm', done: true },
]

const toneMeta: Record<EchoItem['tone'], { label: string; style: { backgroundColor: string; borderColor: string } }> = {
  echo: { label: 'echo', style: { backgroundColor: '#171d29', borderColor: '#b7c3f0' } },
  warmth: { label: 'warmth', style: { backgroundColor: '#2d2234', borderColor: '#d8b9c8' } },
  ease: { label: 'ease', style: { backgroundColor: '#1c2129', borderColor: '#b0d8c5' } },
  calm: { label: 'calm', style: { backgroundColor: '#1a1d24', borderColor: '#d9e2ff' } },
}

const prompts = [
  'What gentle phrase keeps returning to you when life feels busy?',
  'Which reassurance helps your mind land a little softer?',
  'What small truth can be repeated with warmth instead of pressure?',
  'How can a tender message echo in the room without becoming heavy?',
]

export default function LightEchoBoard() {
  const [items, setItems] = useState<EchoItem[]>(starterItems)
  const [title, setTitle] = useState('')
  const [tone, setTone] = useState<EchoItem['tone']>('echo')
  ...
  const addItem = () => {
    ...
    setItems((current) => [...current, { id: `echo-${Date.now()}`, title: value, tone, done: false }])
    setTitle('')
  }
  ...
  return (
    <WellnessBoardShell title="Light Echo" subtitle="Echo rhythm" badge="light">
      <View style={styles.progressCard}>
        <View style={styles.progressHeader}>
          <Text style={styles.progressLabel}>Echo rhythm</Text>
      ...
      <View style={styles.list}>
        {items.map((item) => (
          <Pressable key={item.id} onPress={() => toggleItem(item.id)} style={[styles.item, item.done && styles.itemDone]}>
            ...
            <Text style={styles.itemState}>{item.done ? 'echoed' : 'later'}</Text>
          </Pressable>
        ))}
      </View>
      ...
      <View style={styles.form}>
        <View style={styles.optionRow}>
          {(['echo', 'warmth', 'ease', 'calm'] as EchoItem['tone'][]).map((option) => (
        ...
        <TextInput value={title} onChangeText={setTitle} placeholder="Add a repeated reassurance" placeholderTextColor="#8f8393" style={styles.input} />
