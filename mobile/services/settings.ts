...
async function context() {
  ...
  const { data: link, error } = await supabase
    .from('couple_links')
    .select('id,couple_id,inviter_id,accepted_by,status')
...
export async function getSettingsData(): Promise<SettingsData> {
  ...
  const [
    ...
    supabase
      ...
      .select('height_cm,weight_kg,allergies,medications,conditions,data')
      .eq('user_id', user.id)
      .maybeSingle(),
    supabase.from('ai_privacy_settings').select('*').eq('user_id', user.id).maybeSingle(),
    supabase
      .from('user_settings')
      .select('settings,lock_pin_hash')
      .eq('user_id', user.id)
      .maybeSingle(),
    AsyncStorage.getItem('a-little-world-with-us-language'),
    SecureStore.getItemAsync('a-little-world-with-us-pin'),
  ])
  const stored = (userSettings?.settings ?? {}) as {
    biometricEnabled?: boolean
    privacy?: Record<string, boolean>
  }
  return {
    ...
    relationshipStatus: link ? 'Linked and accepted' : 'Not linked',
    health: {
      age: String(health?.data?.age ?? ''),
      weight: health?.weight_kg ? String(health.weight_kg) : '',
      height: health?.height_cm ? String(health.height_cm) : '',
      conditions: (health?.conditions ?? []).join(', '),
      medications: (health?.medications ?? []).join(', '),
      allergies: (health?.allergies ?? []).join(', '),
    },
    privacy: { ...defaultPrivacy, ...stored.privacy, ...(privacy ?? {}) },
    pinEnabled: Boolean(pin),
    biometricEnabled: stored.biometricEnabled ?? false,
    language: language === 'my' ? 'my' : 'en',
  }
}
...
export async function saveHealthProfile(userId: string, health: SettingsData['health']) {
  ...
  if (health.age && (!Number.isInteger(age) || age < 1 || age > 120))
    throw new Error('Age must be between 1 and 120.')
  if (health.weight && (!Number.isFinite(weight) || weight < 1 || weight > 300))
    throw new Error('Weight must be between 1 and 300 kg.')
  if (health.height && (!Number.isFinite(height) || height < 50 || height > 250))
    throw new Error('Height must be between 50 and 250 cm.')
  const { error } = await supabase.from('health_profiles').upsert({
    user_id: userId,
    height_cm: health.height ? height : null,
    weight_kg: health.weight ? weight : null,
...
export async function savePrivacy(userId: string, privacy: Record<string, boolean>) {
  const aiPayload = Object.fromEntries(
    Object.entries(privacy).filter(([key]) => key.startsWith('allow_ai_'))
  )
  const { error } = await supabase
    .from('ai_privacy_settings')
    .upsert({ user_id: userId, ...aiPayload })
  if (error) throw error
...
export async function saveCouple(coupleId: string, anniversary: string, name: string) {
  const { error } = await supabase
    .from('couples')
    .update({ anniversary: anniversary || null, name: name || null })
    .eq('id', coupleId)
  if (error) throw error
}
...
export async function saveLocalSecurity(
  ...
  if (pinEnabled && pin) {
    ...
    await SecureStore.setItemAsync('a-little-world-with-us-pin', hash)
  }
  if (!pinEnabled) await SecureStore.deleteItemAsync('a-little-world-with-us-pin')
  const { data: existing } = await supabase
    .from('user_settings')
    .select('settings')
    .eq('user_id', userId)
  ...
  const { error } = await supabase.from('user_settings').upsert({
    user_id: userId,
    settings: { ...((existing?.settings ?? {}) as object), biometricEnabled },
  })
  if (error) throw error
}
