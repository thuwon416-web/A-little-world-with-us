...
const starterItems: SignalItem[] = [
  {
    id: 'signal-1',
    title: 'Say the thing that makes the room feel safer',
    tone: 'gentle',
    done: true,
  },
  {
    id: 'signal-2',
    title: 'Leave one soft cue where the other can find it',
    tone: 'warm',
    done: false,
  },
  {
    id: 'signal-3',
    title: 'Let the next step be small enough to feel kind',
    tone: 'steady',
    done: true,
  },
]
...
const prompts = [
  'What kind of signal makes the other person feel immediately held?',
  'Which soft cue can be sent without needing a big explanation?',
  'How can we communicate care without creating pressure?',
  'What simple expression would make today feel less heavy?',
]

export default function QuietSignalBoard() {
  const [items, setItems] = useState<SignalItem[]>(starterItems)
  const [title, setTitle] = useState('')
  const [tone, setTone] = useState<SignalItem['tone']>('gentle')
  const [promptIndex, setPromptIndex] = useState(0)

  useEffect(() => {
    try {
      const raw = localStorage.getItem('quiet-signal-board')
      if (raw) {
        const parsed = JSON.parse(raw)
        if (Array.isArray(parsed) && parsed.length) setItems(parsed)
      }

      const savedPrompt = localStorage.getItem('quiet-signal-prompt')
      if (savedPrompt) {
  ...
  useEffect(() => {
    localStorage.setItem('quiet-signal-board', JSON.stringify(items))
  }, [items])

  useEffect(() => {
    localStorage.setItem('quiet-signal-prompt', String(promptIndex))
  }, [promptIndex])
  ...
  const addItem = () => {
    ...
    setItems((current) => [
      ...current,
      { id: `signal-${Date.now()}`, title: value, tone, done: false },
    ])
    setTitle('')
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-[var(--accent-2)]">
          ...
          <h3 className="font-dancing text-2xl">Quiet Signal</h3>
        </div>
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-[var(--card-bg)]/15 p-3">
        <div className="mb-2 flex gap-2 overflow-x-auto">
          {(['gentle', 'warm', 'bright', 'steady'] as SignalItem['tone'][]).map((option) => (
            <button
              key={option}
        ...
        <div className="flex gap-2">
          <input
            ...
            placeholder="Add a quiet signal"
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-gradient-to-r from-[var(--accent-2)] to-[var(--accent-1)] p-3 text-sm text-[var(--text-primary)]/80">
        ...
        <p>
          Some of the strongest love signals are quiet enough to feel safe, not loud enough to feel
          heavy.
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 rounded-2xl border border-[var(--accent-1)]/30 bg-[var(--bg-2)] p-3 text-sm text-[var(--accent-1)]">
        ...
        <span>Let care be felt without needing to be demanded.</span>
      </div>
    </div>
  )
}
