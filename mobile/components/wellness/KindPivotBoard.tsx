...
const starterItems: PivotItem[] = [
  { id: 'pivot-1', title: 'Notice when the energy needs a kinder adjustment', tone: 'pivot', done: true },
  { id: 'pivot-2', title: 'Let one small shift soften the moment', tone: 'warmth', done: false },
  { id: 'pivot-3', title: 'Return to care before trying to fix everything', tone: 'ground', done: true },
]

const toneMeta: Record<PivotItem['tone'], { label: string; backgroundColor: string; borderColor: string; color: string }> = {
...
export default function KindPivotBoard() {
  ...
  const addItem = () => {
    ...
    if (!value) return
    setItems((current) => [...current, { id: `pivot-${Date.now()}`, title: value, tone, done: false }])
    setTitle('')
  }
  ...
  return (
    <WellnessBoardShell title="Kind Pivot" subtitle="Shift rhythm" badge="pivot">
      ...
      <View style={styles.form}>
        ...
        <TextInput value={title} onChangeText={setTitle} placeholder="Add a kind shift" placeholderTextColor="#8f8393" style={styles.input} />
