...
export default function WatchTogetherPage() {
  ...
  return <main className="mx-auto max-w-7xl space-y-6 p-4 md:p-8">
    ...
    {error && <p className="rounded-xl bg-[var(--error)]/10 p-3 text-[var(--error)]">{error}</p>}
    <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
      <section className="space-y-4">
        ...
        <form onSubmit={(event) => void add(event)} className="glass-card grid gap-3 rounded-2xl p-4 md:grid-cols-[1fr_1fr_auto]"><input value={url} onChange={(event) => setUrl(event.target.value)} placeholder="YouTube URL or video ID" className="rounded-lg border border-[var(--card-border)] bg-[var(--bg-color)]/20 px-3 py-2" /><input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="Title (optional)" className="rounded-lg border border-[var(--card-border)] bg-[var(--bg-color)]/20 px-3 py-2" /><button className="rounded-lg border border-[var(--accent-1)] bg-transparent px-4 py-2 text-[var(--accent-1)] hover:bg-[var(--accent-1)]/10" type="submit">Add video</button></form>
      </section>
      <aside className="space-y-4"><div className="glass-card rounded-2xl p-4"><h2 className="mb-3 text-xl font-semibold">Watchlist</h2><div className="space-y-3">{items.map((item) => <div key={item.id} className="flex items-center gap-3"><img src={item.thumbnail_url ?? getYouTubeThumbnail(item.youtube_id)} alt="" className="h-12 w-20 rounded object-cover" /><button className="min-w-0 flex-1 text-left" onClick={() => void select(item)}><span className="block truncate font-medium">{item.title}</span><span className="text-xs text-[var(--muted)]">Play together</span></button><button className="text-xs text-[var(--error)]" onClick={() => void supabase.from('watchlist').delete().eq('id', item.id).then(() => setItems((old) => old.filter((row) => row.id !== item.id)))}>Remove</button></div>)}</div></div><div className="glass-card max-h-[500px] overflow-auto rounded-2xl p-3"><h2 className="mb-2 px-2 text-xl font-semibold">Party chat</h2><RealtimeChat /></div></aside>
    </div>
  </main>
}
