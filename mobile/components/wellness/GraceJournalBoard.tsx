...
const starterItems: JournalItem[] = [
  ...
  { id: 'grace-3', title: 'A quiet moment that helped us feel restored', kind: 'restore', done: true },
]

const kindMeta: Record<JournalItem['kind'], { label: string; backgroundColor: string; borderColor: string; color: string }> = {
  ...
  restore: { label: 'restore', backgroundColor: '#201d29', borderColor: '#d8b9c8', color: '#f0c9d9' },
...
export default function GraceJournalBoard() {
  ...
  const addItem = () => {
    ...
    if (!value) return
    setItems((current) => [...current, { id: `grace-${Date.now()}`, title: value, kind, done: false }])
    setTitle('')
  }
      ...
      <View style={styles.form}>
        <View style={styles.toneRow}>
          {(['grace', 'wish', 'restore', 'notice'] as JournalItem['kind'][]).map((option) => (
            <Pressable key={option} onPress={() => setKind(option)} style={[styles.modeButton, kind === option && styles.modeButtonActive]}>
              <Text style={[styles.modeText, kind === option && styles.modeTextActive]}>{option}</Text>
            </Pressable>
          ))}
        </View>

        <TextInput value={title} onChangeText={setTitle} placeholder="Write a small note of grace" placeholderTextColor="#8f8393" style={styles.input} />
