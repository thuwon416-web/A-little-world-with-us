...
export async function getCheckins() { const { coupleId } = await context(); const { data, error } = await supabase.from('safety_checkins').select('id, couple_id, user_id, checkin_type, status, message, latitude, longitude, accuracy, expected_until, acknowledged_at, acknowledged_by, completed_at, created_at, updated_at').eq('couple_id', coupleId).order('created_at', { ascending: false }); if (error) throw error; return ((data ?? []) as CheckinRow[]).map(mapCheckin) }
export async function createCheckin(input: SafetyCheckinInput) { const { coupleId, userId } = await context(); const { data, error } = await supabase.from('safety_checkins').insert({ couple_id: coupleId, user_id: userId, checkin_type: input.checkinType, message: input.message, latitude: input.latitude, longitude: input.longitude, accuracy: input.accuracy, expected_until: input.expectedUntil }).select('id, couple_id, user_id, checkin_type, status, message, latitude, longitude, accuracy, expected_until, acknowledged_at, acknowledged_by, completed_at, created_at, updated_at').single(); if (error) throw error; return mapCheckin(data as CheckinRow) }
