...
export default function Categories({ coupleId, onOpenCategory }: { coupleId: string; onOpenCategory?: (category: string) => void }) {
  ...
  return <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{Object.entries(stats).sort(([, a], [, b]) => b - a).map(([category, count]) => { const Icon = icons[category] ?? MessageCircle; return <div key={category} className="rounded-3xl border border-[var(--accent-1)]/15 bg-[var(--card-bg)] p-5 shadow-lg"><button type="button" onClick={() => void toggle(category)} className="flex w-full items-center gap-3 text-left"><Icon size={24} aria-hidden="true" className="shrink-0 text-[var(--accent-1)]" /><span className="flex-1 text-sm font-medium capitalize text-[var(--text-primary)]">{category.replace(/_/g, ' ')}</span><span className="text-sm text-[var(--text-secondary)]">{count}</span></button>{expanded === category ? <div className="mt-4 space-y-3">{memories.map((memory) => <MemoryCard key={memory.id} memory={memory} />)}{onOpenCategory ? <button type="button" onClick={() => onOpenCategory(category)} className="text-sm text-[var(--accent-1)]">View all in this category</button> : null}</div> : null}</div> })}</div>
}
