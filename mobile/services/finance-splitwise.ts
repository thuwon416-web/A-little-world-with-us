...
export type UpdateExpenseInput = Partial<CreateExpenseInput> & { isSettled?: boolean }
...
type ExpenseRow = {
  id: string; couple_id: string; user_id: string; title: string
  amount: number | string; spent_at: string; category: string
  paid_by: string | null; split_type: SplitType; split_with: string | null
  split_percentage: number | string | null; is_settled: boolean
  notes: string | null; created_at: string; updated_at: string
}

type SettlementRow = {
  id: string; couple_id: string; from_user: string; to_user: string
  amount: number | string; notes: string | null; settled_at: string; created_at: string
}
...
function mapExpense(row: ExpenseRow): Expense {
  return {
    id: row.id, coupleId: row.couple_id, userId: row.user_id, title: row.title,
    amount: Number(row.amount), spentAt: row.spent_at, category: row.category,
    paidBy: row.paid_by, splitType: row.split_type, splitWith: row.split_with,
    splitPercentage: row.split_percentage === null ? null : Number(row.split_percentage),
    isSettled: row.is_settled, notes: row.notes, createdAt: row.created_at, updatedAt: row.updated_at,
  }
}

function mapSettlement(row: SettlementRow): Settlement {
  return {
    id: row.id, coupleId: row.couple_id, fromUser: row.from_user, toUser: row.to_user,
    amount: Number(row.amount), notes: row.notes, settledAt: row.settled_at, createdAt: row.created_at,
  }
}

export async function getExpenses(): Promise<Expense[]> {
  const { coupleId } = await getContext()
  const { data, error } = await supabase.from('finance_expenses').select('*').eq('couple_id', coupleId)
...
export async function createExpense(input: CreateExpenseInput): Promise<Expense> {
  ...
  const { data, error } = await supabase.from('finance_expenses').insert({
    couple_id: coupleId, user_id: userId, title: input.title, amount: input.amount,
    spent_at: input.spentAt, category: input.category ?? 'other', paid_by: input.paidBy ?? userId,
    split_type: input.splitType ?? 'equal', split_with: input.splitWith ?? null,
    split_percentage: input.splitPercentage ?? null, notes: input.notes ?? null,
  }).select('*').single()
  if (error) throw new Error(error.message)
...
export async function updateExpense(id: string, input: UpdateExpenseInput): Promise<void> {
  ...
  const { error } = await supabase.from('finance_expenses').update(updates).eq('id', id).eq('couple_id', coupleId)
  if (error) throw new Error(error.message)
}

export async function deleteExpense(id: string): Promise<void> {
  const { coupleId } = await getContext()
  const { error } = await supabase.from('finance_expenses').delete().eq('id', id).eq('couple_id', coupleId)
  if (error) throw new Error(error.message)
}
...
export async function getSettlements(): Promise<Settlement[]> {
  const { coupleId } = await getContext()
  const { data, error } = await supabase.from('settlements').select('*').eq('couple_id', coupleId)
...
export async function createSettlement(input: CreateSettlementInput): Promise<Settlement> {
  ...
  const { data, error } = await supabase.from('settlements').insert({
    couple_id: coupleId, from_user: input.fromUser, to_user: input.toUser,
    amount: input.amount, notes: input.notes ?? null, settled_at: input.settledAt,
  }).select('*').single()
  if (error) throw new Error(error.message)
  return mapSettlement(data as SettlementRow)
}

export function calculateBalance(expenses: Expense[], settlements: Settlement[], myUserId: string, partnerUserId: string): BalanceSummary {
  let netBalance = 0
  let totalExpenses = 0
  let expensesCount = 0
  ...
  return {
    netBalance, theyOweYou: Math.max(0, netBalance), youOweThem: Math.max(0, -netBalance),
    totalExpenses, expensesCount, settlementsCount: settlements.length,
  }
}
