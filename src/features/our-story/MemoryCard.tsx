      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-center gap-3">
          {(() => {
            ...
            return <Icon size={24} aria-hidden="true" className="shrink-0 text-accent-1" />
          })()}
          <div className="min-w-0">
            <p className="truncate text-sm font-medium text-text-1">
              {formatCategory(memory.category)}
            </p>
            <p className="text-xs text-text-2">{formatDate(memory.date_time)}</p>
          </div>
        </div>
        {memory.importance === 'critical' || memory.importance === 'high' ? (
          <span className="rounded-full bg-accent-1/15 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide text-accent-1">
            {memory.importance}
          </span>
        ) : null}
      </div>
      {memory.quote_burmese ? (
        <p className="mt-4 text-base leading-relaxed text-text-1">{memory.quote_burmese}</p>
      ) : null}
      {memory.context ? (
        <p className="mt-2 text-sm leading-relaxed text-text-2">{memory.context}</p>
      ) : null}
    </article>
  )
}
