...
export default function AuthGuard({ children }: { children: React.ReactNode }) {
  ...
  if (authError) {
    return (
      <div className="flex min-h-screen items-center justify-center p-6">
        <div className="max-w-md rounded-[2rem] border border-error/20 bg-card p-6 text-center shadow-xl backdrop-blur-xl">
  ...
  if (!isAuth) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-bg dark:bg-[var(--bg-2)]">
        <div className="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-accent-1" />
      </div>
    )
  }
  ...
  if (pairStatus && pairStatus !== 'accepted') {
    return (
      <div className="flex min-h-screen items-center justify-center p-6">
        <div className="max-w-md rounded-[2rem] border border-accent-1/20 bg-card p-6 text-center shadow-xl backdrop-blur-xl">
          <p className="text-sm uppercase tracking-[0.22em] text-text-2">Private access</p>
          <h1 className="mt-3 text-3xl text-text-1" style={{ fontFamily: 'var(--font-display)' }}>
