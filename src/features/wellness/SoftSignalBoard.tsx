...
import { BellRing, Heart, Plus, Sparkles, Wind } from 'lucide-react'

type SignalItem = {
  id: string
  title: string
  tone: 'signal' | 'warmth' | 'ease' | 'calm'
  done: boolean
}
...
const starterItems: SignalItem[] = [
  {
    id: 'signal-1',
    title: 'Let one small cue mean more than a long explanation',
    tone: 'signal',
    done: true,
  },
  {
    id: 'signal-2',
    title: 'Use a softer phrase before a stronger point',
    tone: 'warmth',
    done: false,
  },
  {
    id: 'signal-3',
    title: 'Notice the room before responding to the moment',
    tone: 'calm',
    done: true,
  },
]
...
const prompts = [
  'What small signal tells the other person, “I am with you”?',
  'Which tone helps the message land softly instead of sharply?',
  'How can we say what matters without making the room feel heavier?',
  'What gentle cue makes communication feel safer and less rushed?',
]

export default function SoftSignalBoard() {
  const [items, setItems] = useState<SignalItem[]>(starterItems)
  const [title, setTitle] = useState('')
  const [tone, setTone] = useState<SignalItem['tone']>('signal')
  const [promptIndex, setPromptIndex] = useState(0)

  useEffect(() => {
    try {
      const raw = localStorage.getItem('soft-signal-board')
      if (raw) {
        const parsed = JSON.parse(raw)
        if (Array.isArray(parsed) && parsed.length) setItems(parsed)
      }
      const savedPrompt = localStorage.getItem('soft-signal-prompt')
      if (savedPrompt) {
  ...
  useEffect(() => {
    localStorage.setItem('soft-signal-board', JSON.stringify(items))
  }, [items])

  useEffect(() => {
    localStorage.setItem('soft-signal-prompt', String(promptIndex))
  }, [promptIndex])
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-[var(--accent-2)]">
          ...
          <h3 className="font-dancing text-2xl">Soft Signal</h3>
        </div>
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-[var(--card-bg)]/15 p-3">
        <div className="mb-2 flex gap-2 overflow-x-auto">
          {(['signal', 'warmth', 'ease', 'calm'] as SignalItem['tone'][]).map((option) => (
            <button
              key={option}
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-gradient-to-r from-[var(--accent-2)] to-[var(--accent-1)] p-3 text-sm text-[var(--text-primary)]/80">
        ...
        <p>
          Soft signals are often quieter than the message itself, yet they are the ones that make a
          person feel seen.
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 rounded-2xl border border-[var(--accent-1)]/30 bg-[var(--bg-2)] p-3 text-sm text-[var(--accent-1)]">
        ...
        <span>Care can speak softly and still land deeply.</span>
      </div>
    </div>
  )
}
