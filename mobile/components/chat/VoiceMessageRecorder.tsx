...
export function VoiceMessageRecorder({ visible, onClose, onRecord }: Props) {
  ...
  const stop = async () => {
    ...
    try {
      ...
      Alert.alert('Voice note failed', error instanceof Error ? error.message : 'Unable to send voice note.')
    } finally {
      setSaving(false)
    }
  }
  ...
  return (
    <Modal visible={visible} transparent animationType="fade" onRequestClose={onClose}>
      <View style={styles.overlay}><View style={styles.card}>
        <View style={styles.header}><Text style={styles.title}>Voice note</Text><TouchableOpacity onPress={onClose}><X color="#f3f0f5" size={22} /></TouchableOpacity></View>
        <Text style={styles.timer}>{Math.floor(duration / 60)}:{String(duration % 60).padStart(2, '0')}</Text>
        <TouchableOpacity style={[styles.record, recording && styles.stop]} onPress={() => void (recording ? stop() : start())} disabled={saving}>
