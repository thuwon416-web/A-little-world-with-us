...
import { Compass, Heart, Plus, Sparkles, SunMedium } from 'lucide-react'

type HorizonItem = {
  id: string
  title: string
  tone: 'horizon' | 'warmth' | 'ease' | 'ground'
  done: boolean
}

const starterItems: HorizonItem[] = [
  {
    id: 'horizon-1',
    title: 'Let the next plan be gentle enough to breathe',
    tone: 'ground',
    done: true,
  },
  {
    id: 'horizon-2',
    title: 'Choose one small wish for the coming week',
    tone: 'warmth',
    done: false,
  },
  {
    id: 'horizon-3',
    title: 'Keep the future soft enough to feel welcome',
    tone: 'ease',
    done: true,
  },
]
...
const prompts = [
  'What future feeling would make the next week feel lighter?',
  'Which tiny wish would bring more ease into the days ahead?',
  'What gentle plan lets both of you feel steady and seen?',
  'How can we build tomorrow without forgetting softness today?',
]

export default function GentleHorizonBoard() {
  const [items, setItems] = useState<HorizonItem[]>(starterItems)
  const [title, setTitle] = useState('')
  const [tone, setTone] = useState<HorizonItem['tone']>('horizon')
  const [promptIndex, setPromptIndex] = useState(0)

  useEffect(() => {
    try {
      const raw = localStorage.getItem('gentle-horizon-board')
      if (raw) {
        const parsed = JSON.parse(raw)
        if (Array.isArray(parsed) && parsed.length) setItems(parsed)
      }
      const savedPrompt = localStorage.getItem('gentle-horizon-prompt')
      if (savedPrompt) {
  ...
  useEffect(() => {
    localStorage.setItem('gentle-horizon-board', JSON.stringify(items))
  }, [items])

  useEffect(() => {
    localStorage.setItem('gentle-horizon-prompt', String(promptIndex))
  }, [promptIndex])
  ...
  const addItem = () => {
    ...
    setItems((current) => [
      ...current,
      { id: `horizon-${Date.now()}`, title: value, tone, done: false },
    ])
    setTitle('')
  }
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-[var(--accent-2)]">
          ...
          <h3 className="font-dancing text-2xl">Gentle Horizon</h3>
        </div>
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-[var(--card-bg)]/15 p-3">
        <div className="mb-2 flex gap-2 overflow-x-auto">
          {(['horizon', 'warmth', 'ease', 'ground'] as HorizonItem['tone'][]).map((option) => (
            <button
              key={option}
      ...
      <div className="rounded-2xl border border-[var(--accent-1)]/20 bg-gradient-to-r from-[var(--accent-2)] to-[var(--accent-1)] p-3 text-sm text-[var(--text-primary)]/80">
        ...
        <p>
          A kind future is not rigid. It is a horizon wide enough for both of you to feel safe
          stepping into it.
        </p>
      </div>
      ...
      <div className="flex items-center gap-2 rounded-2xl border border-[var(--accent-1)]/30 bg-[var(--bg-2)] p-3 text-sm text-[var(--accent-1)]">
        ...
        <span>Dream gently, and let the future meet you softly.</span>
      </div>
    </div>
  )
}
